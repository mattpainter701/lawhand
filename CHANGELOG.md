## 2026.09.26.01 — PDF preparation polish and safer packet retries

- `GeneratedPdfPreview` adds a **Fit page** zoom mode, observes both preview width and height, and uses a stable 60dvh review pane with contained scrolling. PDF previews used from Prepare, matter-document previews, sample fill and dialogs therefore keep a predictable canvas instead of shrinking into a cramped plugin-like area.
- `TemplateFillSource` now sizes PDF source pages from the available pane and uses rotation-aware page dimensions for fallback layout. Clickable field overlays scale with the rendered page on narrower screens.
- `TemplatesPage` global-library import shows an explicit "Shared source selected" state for the preloaded sample file, hides the dropzone until the user asks to choose a different source, and labels PDF setup as **Save draft** followed by **Review fields in editor**. PDF scans no longer appear to have completed source review just because analysis finished.
- Authored global-library starter forms now carry conservative provenance in the manifest (`source_name = "LawHand authored sample"` and source file paths) generated by `backend/scripts/build_library_intake_forms.py`. The committed PDF bytes, field names, bindings and wording are unchanged.
- `fill_sessions.enqueue_render` rejects duplicate template members before mutating the session or queueing a job. Queued packet-save jobs now carry the session answer digest and job id; `run_set_render_job` treats jobs whose session is no longer saving under that exact job/digest as stale no-ops, so an old replay cannot overwrite a newer or completed fill session.
- Tests cover duplicate member rejection, stale packet replay preservation, PDF Fit page sizing, source overlay scaling, global-library import copy, authored-form provenance, and the updated release catalog.
