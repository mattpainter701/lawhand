## 2026.09.24.01 — Microsoft 365 and Google connection clarity
...
- Teams panel no longer offers a reconnect for personal Microsoft accounts (`capabilities.teams` unavailable with a known tier) or when every Teams scope is granted but the feature is disabled; the hub pill separates an unconfirmed tier from an unsupported one. The overview hero heading is visible again, and the section nav sets `aria-current`.
