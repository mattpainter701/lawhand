## Unreleased — Remove the BYOK customer-LLM path (D68, D69)

- Product decision: LawHand never calls a firm's own model provider. Firms connect LawHand's platform and research MCP servers to the AI tool they already pay for. The API-only "bring your own key" path had no UI and no customer, so it is removed rather than rebuilt.
- Removed `POST`/`DELETE /api/admin/customer-llm/configure` and `CustomerLLMConfigRequest` (`app/routers/admin.py`), `app/services/byok_security.py`, the `resolved_route="customer"` branch in `llm_routing.resolve_llm_route` (with its hard-coded `gemini-2.0-flash` default) and the `customer_api_key`/`customer_provider`/`customer_endpoint` fields on `LLMRoute`. Every tenant now resolves to the platform gateway routes.
- `app/services/llm.py`: `_client_for` is gone, so every completion and stream uses the LiteLLM gateway client and always sends gateway metadata and `stream_options.include_usage`. Callers (`chat.py`, `chat_agent.py`, `ai_request_broker.py`, `office_assistant.py`, `matter_document_revisions.py`, `template_ai_service.py`) no longer pass customer credentials.
- Billing: the branches that recorded customer-route usage at $0 (`chat.py` answer, stream and chat-action usage, `office_assistant` and `matter_document_revisions` usage records) are removed; platform-route billing is unchanged.
- Removed the unused `AZURE_OPENAI_ENDPOINT`/`AZURE_OPENAI_KEY`/`AZURE_OPENAI_DEPLOYMENT` and `GEMINI_API_KEY` settings, and `configureCustomerLLM`/`resetCustomerLLM` from `frontend/src/api.js` (no component imported them).
- Migration `204_retire_customer_llm` clears `tenant_settings.use_customer_llm`, `customer_llm_provider` and `customer_llm_config` (the encrypted provider key, endpoint and deployment) with a scoped update inside a NO FORCE RLS window. The downgrade is a no-op because the wiped keys cannot be restored. The columns stay mapped but unread until a later contract migration drops them; `demo_clone`, the demo routes, the demo seed and `rotate_token_encryption` no longer read or write them.
- Tests: `test_chat.py` seeds a tenant still flagged for BYOK and checks Standard and Premium resolve to the gateway aliases; `test_migrations.py` checks the migration's statements and runs its update against seeded rows; `test_onboarding.py` checks the admin routes are gone. BYOK-only tests are deleted.
- No customer-facing release note: no UI exposed the feature.

## Unreleased — Narrower Microsoft consent and accurate permission disclosures (D28, D81)

- D28, Teams: `app/services/teams.py` `TEAMS_REQUIRED_SCOPES` drops `Chat.ReadWrite` and `TeamsActivity.Send`. Nothing calls `/chats` or `sendActivityNotification`, and the Teams manifest declares no activity types; the code uses only `/me/joinedTeams`, `/teams/{id}/channels` and channel `messages`. The Teams opt-in (`TEAMS_CONNECT_SCOPES`) now asks for `Channel.ReadBasic.All ChannelMessage.Send Team.ReadBasic.All Channel.Create`. Grants stored before this are a superset, so `teams_gate` keeps reporting `teams_connected` and a re-authorize without `&teams=1` still adds the Teams set.
- D28, per-user files: `MICROSOFT_USER_SCOPES` requests `Files.Read.All` instead of `Files.ReadWrite.All`. Every Graph file write (matter upload, replace, delete and trash, folder creation, folder marker, folder share and unshare, Open in Word reconcile and revised version, assistant draft saves) resolves the tenant credential through `get_fresh_token`; per-user Microsoft tokens only list, search and download. New `integration_observability.MICROSOFT_SCOPE_ALIASES` / `microsoft_scope_granted` let a stored `Files.ReadWrite.All` satisfy `Files.Read.All` (never the reverse), used by `integrations._scope_is_granted` and the `/api/auth/calendar-providers` `missing_features` check (`PERSONAL_CONNECTION_SCOPES["microsoft"]["files"]` is now `Files.Read.All`). Existing personal connections are not flagged "missing scopes" and need no re-consent; Microsoft also reports previously consented scopes on a reconnect, which the alias covers.
- D28, sign-in: Microsoft sign-in (`/api/auth/microsoft/login` and its token exchange) requests `openid email profile User.Read` (`MICROSOFT_SIGN_IN_SCOPE`) without `offline_access`. The callback only checks that an access token exists and keeps the verified id_token; no refresh token was ever read.
- Deliberately unchanged: the tenant `Mail.Read` scope and the admin-mailbox indexing fallback in `cloud_sync`, pending the D02/D03 product decision.
- D81: the onboarding wizard's "Read all users, read mail, read files" line is replaced by the consent list rendered from `components/integrationScopeLabels.js`, the labels the admin integrations card already used (moved out of `IntegrationsPanel.jsx`), applied to the published scope matrix. It now names sending mail and file write access. The Integrations hub cloud card names sending and file writes. The profile Connected accounts card no longer promises to "save" Microsoft files. `/requirements` (page and server shell) says per-user Microsoft consent reads files only and what the Teams opt-in covers.
- D81 docs: the admin guide "Integration permissions and data visibility" lists `Mail.Send`, `gmail.send`, the sign-in, per-user and Teams scopes, and no longer says Gmail is read-only or "does not authorize sending"; the Teams admin guide lists the three Teams permissions; the user guide "What connected integrations can view" adds sending, explains that personal Microsoft file access is read-only, and replaces the "not copied into a LawHand datastore" claim with the synced copies and the local-save fallback. `docs/integrations-setup.md` and `docs/virtual-document-assistant-plan.md` state the send scopes. `frontend/src/marketing/integration-scopes.json` matches the new constants.
- Tests: `test_teams.py` (dropped scopes, a legacy superset grant passes the gate and status, re-authorize no longer asks for them), `test_oauth_callback_contract.py` (alias in both directions, per-user callback audit for new, legacy and declined grants, calendar-providers accepts either file scope), `test_microsoft_sign_in_scope.py` (authorize URL and token exchange), `test_public_integration_scopes.py` (the admin guide must name every requested scope and no unrequested Graph permission), and Vitest for the onboarding list, hub card and connected accounts copy.

## Unreleased — Recoverable Google Drive deletes and safe file names in Graph uploads (D36, D37)

- `app/routers/matter_documents.py` (`_delete_cloud_provider_object`): deleting a Google Drive matter document now moves the file to the Drive trash (`PATCH files/{id}` with `{"trashed": true}`, `supportsAllDrives=true`, `fields=id,trashed`) instead of `DELETE files/{id}`. The old call omitted `supportsAllDrives`, so Drive answered 404 for every Shared Drive item, the 404 was read as "already gone", and LawHand dropped its record while the file stayed in the drive. My Drive deletes were permanent; they are now recoverable from the trash, as OneDrive/SharePoint deletes already are from the recycle bin. Trashing needs only the Content manager (`fileOrganizer`) role the service account holds on the Shared Drive. A 403 from Drive now returns a 502 that names the refused trash move and the role needed, logs a warning, and keeps the database record; a 200 that does not report `trashed: true` also fails with 502. A 404 still counts as already deleted, now that the request can see Shared Drive items.
- `app/services/matter_file_store.py`: the file name is percent-encoded (`quote(name, safe="")`, new `_graph_path_segment`) in every Graph path-addressed upload URL: the OneDrive simple upload (`items/{parent}:/{name}:/content`), the OneDrive `createUploadSession` URL and the SharePoint upload. `#` used to cut the path and drop `conflictBehavior=rename`, `?` started a query and `%` was read as an escape, so names such as "Exhibit #3.pdf" failed to upload (inbound email attachments were skipped with only a log line). The upload-session JSON body still carries the raw name, and Graph decodes the path, so the stored and displayed name keeps its original characters.
- `app/services/document_sync.py`: the OneDrive folder listing (`root:{path}:/children`) encodes each path segment the same way.
- Tests: `test_workspace_mcp_changed_router_contracts.py` runs the delete against a fake Drive endpoint with Shared Drive semantics (Shared Drive and My Drive items are trashed, an already-gone item succeeds, a refused trash surfaces as 502 and keeps the record). `test_matter_file_store.py` checks the exact URL sent for names with `#`, `%`, `?`, a literal `%20`, spaces and non-ASCII characters on all three upload paths.

## Unreleased — Leaving a matter removes the cloud folder share (D07)

- Security: assigning someone to a matter shares its OneDrive and Google Drive folders with them (write on the matter folder, read on linked context folders), but nothing took the share back. Removing the assignment, or deactivating the person, left their provider permission in place, so someone taken off or walled off from a matter kept access to its files.
- New table `matter_folder_share_grants` (migration `203_matter_folder_share_grants`, RLS forced) records each permission LawHand creates with the provider's permission id. `cloud_init.share_matter_folders` now invites one person per call, reads the folder's permissions first, and records `origin`: `lawhand` (created by LawHand), `preexisting` (the person already held a direct share with a different role that the provider merged the invite into), or `legacy` (no id returned). The add-assignment route now shares the matter's folders with the new assignee (it previously shared nothing), and re-shares skip deactivated users.
- `DELETE /api/matters/{id}/assignments/{aid}` and `DELETE /api/admin/users/{id}` mark the person's grants `revoke_pending` in the same transaction, then delete exactly the stored permission (`DELETE /me/drive/items/{id}/permissions/{pid}`, `DELETE /files/{id}/permissions/{pid}?supportsAllDrives=true`) after the commit. A share with no stored id, including every share made before this change, is matched on the person's email plus the exact role LawHand grants on that folder. Inherited and owner permissions are never touched, and `preexisting` shares are marked `kept` and left alone. A folder another matter still shares with the person is not removed. Reactivating a user shares their assigned matters' folders again.
- Fail safe: a provider failure (network, 403, expired or missing token) never blocks the unassign or deactivation. It is recorded as a `cloud_folder_unshare_failed` matter event, a failed `matter_folder_unshare` integration sync run (shown under the provider on the admin permissions page) and an `integration_sync_error` in the error log. A durable `matter_folder_unshare` job retries with backoff (12 attempts), and an hourly `matter-folder-unshare-sweep` re-queues anything still pending. A person re-assigned before the retry keeps their access.
- The "working on" toggle (`PATCH …/assignments/{aid}/active`) is a personal status flag, not an access change, so it keeps the share. SharePoint-bound matter folders are unaffected: access there follows site membership and LawHand grants nothing per person.
- Fixes `POST /api/matters/{id}/assignments`, which returned 500 because its response omitted `is_active_working`.
- Tests: `test_matter_folder_unshare.py` runs the routes against an in-memory Graph and Drive permission API.

## Unreleased — Excel values cannot carry formulas; date-only tasks reach Google Calendar

- D19 (security): `app/services/office_action_policy.py` now refuses a `set_selected_values` string whose first non-whitespace character is `=`, `+`, `-` or `@` with `OfficePolicyError("formula_in_values")`, because Excel parses such a string written through `range.values` as a formula and it would skip the `_UNSAFE_FORMULA_RE` ban (`=WEBSERVICE(…)`, `+HYPERLINK(…)`, `=[Book1]Sheet1!A1`). Plain numbers written as strings (`"-5"`, `"+1.5"`, `"-1e3"`) and JSON numbers still pass; formulas must use `set_selected_formulas`. The Office add-in's `validatePlan` (`office-addin/src/contracts/validation.ts`) applies the same rule before `range.values` is written, and the assistant prompt tells the model about it.
- D12: `app/services/google_calendar._google_schedule` sends a date-only task as an all-day event whose `end.date` is the next day. Google's end date is exclusive, so the previous `end == start` was an empty range that Google rejects. A PATCH of an existing event now nulls the form it no longer uses (`dateTime`/`timeZone` for all-day, `date` for timed), because Google merges nested objects on PATCH. The Microsoft Graph path already used an exclusive next-day end and is unchanged.
- Tests: `test_office_assistant.py` (refused and allowed Excel values), `test_task_calendar_due_time.py` (next-day end, month/year/leap rollover, POST and PATCH bodies), `office-addin/tests/validation.test.ts` (add-in rule).
...
- `correspondence_capture._matter_party_addresses` no longer adds verified send-as aliases of assigned staff. A staff address is on nearly every message in that person's mailbox, so the scheduled sweep filed unrelated mail into every matter they were assigned to. `tracked_addresses` on the rules panel now lists client and party contacts only.
- Case-number matching is whole-token and separator-tolerant (`case_number_mentioned`): `2024-CV-1234` matches "2024 cv 1234" and "2024.CV.1234" but no longer matches inside "2024-CV-12345", and a matter numbered `2024-CV-12` no longer captures mail about `2024-CV-1234`.
- `narrow_to_case_number_matches`: when one email matches several matters in the sweep (a client with several open matters) and names one of their case numbers, it is filed only to those matters. With no case number the party matches stand as before.
- Cross-mailbox dedup: `ms_read_mail_user` selects `internetMessageId` and `gmail_read_mail` requests the `Message-ID` header, both returned as `internet_message_id`. Captures store it in `participants.message_id`, and `_already_captured` checks it alongside `external_ref`, so the same message in two staff mailboxes is filed once. Rows captured before this change carry no Message-ID and are not back-filled.
- Mailbox triage (`email_agent._match_email_to_matters`) returns matters in a meaningful order: matters whose case number the message names first (and only those), otherwise most recently updated first. The communication log and any tagged task previously went to an arbitrary first element of a set.
- `/api/communications` list, detail and PATCH responses, and `/api/contacts/{id}/communications`, add `matter_name`, `matter_number`, `contact_name`, `document_id` and `content_locked`. `content_locked` is true for rows with an `external_ref`, a stored document, a `portal`/`sms` channel or a status other than `logged`.
- `PATCH /api/communications/{id}` refuses (409) changes to `subject`/`body`/`summary`/`status` on content-locked rows; re-filing (`matter_id`, `contact_id`) is still allowed. Re-filing now requires access to the target matter (404 otherwise) and a contact in the tenant.
- Soft-deleted communications no longer appear in `/api/contacts/{id}/communications` or `/api/matters/{id}/correspondence`. The matter correspondence `participant` filter matches from/to/cc only.
- `CommunicationsPage`: rows show the matter name and number (linking to the matter's Correspondence or Activity tab) and the contact name instead of UUID fragments. Clicking a subject expands the message with **Download original** when a stored copy exists. Captured rows show **Move to another matter** (matter and contact pickers only) instead of **Edit**. Hand-logged rows keep **Edit** and send only changed fields. The matter and contact UUID filters are replaced by `MatterPicker` and `ContactPicker`. The never-stored **Note** channel filter is removed, and the delete confirmation no longer claims a permanent delete. `MatterPicker` gains `label` and `inputId` props.
- No migration.

## 2026.09.25.08 — Use a global library form on a matter without importing it

- New `POST /api/templates/library/{sample_id}/save-to-matter` (`manage_documents`): renders the library PDF with `enforce_required=True` and `flatten=True`, stores it through `matter_file_store` (routed by the optional `folder_id`, which must belong to the matter), and adds a `MatterDocument` (category `generated`, `generation_summary.source = "global_library"` with `sample_template_id`) plus a `document_generated` `MatterEvent` carrying the sample id, source name/edition, source SHA-256 and output SHA-256. A failed flush removes the staged file; a failed commit checks the outcome through `_matter_document_commit_outcome` before removing anything, as the firm-template render does. Nothing is written to the firm template library.
- `MatterTemplatePicker`: a global form with a field schema gets **Use on this matter**, which opens `SampleFillDialog` for that matter and folder; **Add to firm** is renamed **Add to firm library** and becomes the secondary action (the only action for a form with no schema). Closing the fill dialog before saving returns to the picker; closing after saving closes the picker.
- `SampleFillDialog` accepts `fixedMatterId`, `folderId` and `onSaved`. With a matter it Smart Fills on open, hides the matter search, and replaces Preview/Download with **Review final PDF** → **Save to matter** (disabled while a required answer is missing). Every field is sent, blank included. After saving it shows the filename with **Download**, **Share with client** (`portal_visible: true`) and **Done**, and ignores further edits. Escape and Close are ignored while a save is in flight.
- New `saveSampleTemplateToMatter` API helper. No migration.
...
- Disconnecting Microsoft 365 or Google now requires typing `disconnect Google` / `disconnect Microsoft 365` (case- and space-insensitive) in a dialog that lists what stops, how many personal connections are removed and what happens at the provider. `ConfirmProvider` gains opt-in `details` and `requireText` options for other wide-impact actions.
- Google re-authorize sends `account_mode=personal` for personal-Gmail tenants (previously always `account_mode_mismatch`), and a solo practice can connect personal Gmail from the panel.
- Profile gains `ConnectedAccountsCard`, built on the existing `GET /auth/calendar-providers`, showing per-user Microsoft/Google status and what the single per-user consent allows (mail read, send, calendar, files).
- Teams panel no longer offers a reconnect for personal Microsoft accounts (`capabilities.teams` unavailable with a known tier) or when every Teams scope is granted but the feature is disabled; the hub pill separates an unconfirmed tier from an unsupported one. The section nav sets `aria-current`.
- Onboarding and the Cloud panel share `utils/oauthErrors.js`, which names the provider that failed and covers `access_denied`, `consent_required`, `interaction_required`, `invalid_state`, `token_exchange_failed` and `identity_verification_failed`.
- `POST /api/email/scan` now returns 403 when the body names another user's `user_id`. It previously scanned that same-tenant user's mailbox with their delegated grant and returned subjects, senders, AI summaries and draft replies to the caller. The SPA never sends `user_id`.
- Google Drive requests made with the platform service account are pinned to the requesting tenant's org Shared Drive (`google_service_account.service_account_drive_scope`). The same service account is a member of every tenant's Shared Drive, so cloud search (`corpora=allDrives`) and the tenant-wide Drive metadata sync (`corpora=user` with `includeItemsFromAllDrives`) could return and index other tenants' files when a user had no personal Google grant. Search and sync now use `corpora=drive&driveId=<tenant drive>`, and content fetch confirms the file's `driveId` first. `prefer_service_account` returns the token as a `ServiceAccountDriveToken` carrying the drive it was chosen for, so the restriction survives a service-account token refresh and reaches the session-free search fan-out; a bare service-account token (one minted by this process without a drive) lists and reads nothing.
- The Microsoft admin connect now also requests `openid email profile` (`MICROSOFT_IDENTITY_SCOPES`; audited Graph scopes unchanged) so Entra returns an id_token. The callback records the tier and granting account from it, and no longer stamps `account_detected_at` with an `unknown` tier when there are no claims, which had made `backfill_unknown_credentials` skip the credential permanently (directory sync blocked, Teams unavailable, "Granted by" missing). The published scope matrix (`integration-scopes.json`) lists the sign-in scopes.
...
- `app/services/mcp_product.py`: `normalize_idempotency_key`, `request_fingerprint`, `lock_mcp_idempotency_key`, and `find_idempotent_usage_event`; `record_mcp_usage` persists the three new fields. An over-long key is rejected (400) rather than truncated, so two keys that differ only past a cut-off cannot collapse into one.
- `app/routers/mcp.py`: a Research call that carries an `Idempotency-Key`/`X-Idempotency-Key` header takes a transaction-scoped advisory lock on `(tenant, credential, key)` and looks up an existing usage event before the proxy. A retry with the same tool and arguments is rejected `409` without re-retrieving or re-billing; reusing a key for different arguments is a `409` conflict. The unique index is the race backstop. Failed calls are not keyed, so a retry after a failure still runs. This closes the retry double-bill found in the audit; billing remains per successful call, so a crash in the narrow post-retrieval window still under-counts rather than billing a failure.
- `app/routers/billing.py`: the billing-page estimate now includes Research OAuth calls alongside product-key calls (was `product_key_id IS NOT NULL`; now product-key or OAuth). Internal `jwt` reads stay excluded, and a new `oauth_calls_30d` field reports the OAuth share.
- Tests: `test_mcp_idempotency_unit.py` (key normalization, argument fingerprint stability, credential scoping, router replay/conflict without a second proxy or charge); `test_mcp_product_gateway.py` (partial unique index per tenant/credential, and OAuth-inclusive billing estimate with `jwt` excluded).

## 2026.09.21.01 — Search inside a matter's documents (Phase 5e) and 2c audit follow-ups

- **Migration `200_matter_document_index`** (`down_revision = 199_workflow_document_requests`): `matter_document_chunks(tenant_id, matter_id, matter_document_id, document_sha256, engine_version, chunk_index, content, fts GENERATED tsvector, embedding vector(1536), created_at)`, unique per document and position, `ON DELETE CASCADE` from the matter document and the matter, GIN index on `fts`, RLS enabled and forced. Applied up, down and up on a scratch database. Registered in `demo_registry` as purge-only.
- **`services/matter_document_index.py`**: `split_text` (character-based, sentence and paragraph aware, overlapping; the token counter in `text_processing.chunk_text` needs a downloaded encoding, and this index must build offline), `index_document` (idempotent per document, digest and engine version; rows for other bytes replaced; embeds through `embed_batch` when an embedder is given and indexes by words only when it fails; commits like the cache write it follows), `forget_document`, `indexed_documents`, and `search` (`websearch_to_tsquery` with `ts_rank_cd` and `ts_headline` snippets, fused by reciprocal rank with the nearest chunks by cosine distance when `embed_text` answers; each hit `{document_id, filename, chunk_index, snippet, score, matched_by, open_url}`). `default_embedder()` builds `EmbeddingService` only when a provider is configured.
- **Hooked into extraction**: `matter_fact_extraction._extract` indexes right after `document_text_cache.get_or_extract`, in a try/except that logs and rolls back, so the index never gates reading a document.
- **Route** `GET /api/matters/{id}/documents/search?q=&limit=` (matter access checked; `{query, results, indexed_documents}`). **MCP**: `GetMatterContextArgs.query` and the `excerpts` section of `get_matter_context`, each snippet wrapped by `wrap_untrusted_text`; without a query the section is empty.
- **Frontend**: the Documents tab's existing search box (three characters, 400 ms pause) also asks the index and shows "Found inside N documents" with the excerpt and an Open link, or says nothing inside the documents mentions it, or that no document text is indexed yet.
- **Audit follow-ups (Phase 2c)**: migration `201_workflow_docdefs_immutable` puts the `prevent_approved_workflow_mutation` trigger on `matter_workflow_document_definitions` like its sibling definition tables (the revision id is short because `alembic_version.version_num` is 32 characters); a "document already saved" rollback blocker is recorded as a blocked `document_propose` step rather than `stage_restore`; the user guide covers documents in progress, documents from a workflow, and search inside documents.
- **Derived document data in its own unit of work.** `document_text_cache.get_or_extract` writes the cache row through its own `session_factory` session and commits there; the caller's transaction is never committed and never has to re-apply its tenant context on account of the cache. The search index is built by a `matter_document_index` durable job (`matter_document_index.enqueue_index`, key `document:digest:engine`, requeued on failure) queued the same way from `matter_fact_extraction._extract`, so a new upload is searchable a few seconds after it is read rather than in the same request, and a queue fault is only logged. `delete_matter_document` calls `document_text_cache.forget_if_unreferenced`, which drops the tenant's cache rows for the digest once no document of the tenant carries it; the search rows cascade. Tests bind both factories to the test engine through an autouse fixture in `conftest.py`.
- Tests: `test_matter_document_index_postgres.py` (splitter, idempotent index and replacement on new bytes, cascade on delete, matter and tenant scoping, words plus meaning with a stand-in embedder and a broken one, route 200/422/404, matter-context excerpts with and without a query, the extraction hook and its fault tolerance, migration text); the saved-document rollback test asserts the step label; vitest Documents tab excerpts case; head pins → 201.

## 2026.09.20.05 — Workflows can request documents (Phase 2c)

- **Migration `199_workflow_document_requests`** (head; `down_revision = 198_document_fill_sessions`; head pins updated): `matter_workflow_document_definitions(tenant_id, template_version_id, stage_key, item_key, document_template_id, title, position, due_offset_days, assignee_role)` with RLS enabled and forced, unique keys and positions per version, and a composite FK to the version's stage; `ck_matter_workflow_run_steps_type` widened with `document_propose`. Downgrade drops the table and re-adds the narrower check `NOT VALID`, because recorded steps are immutable history. Applied up, down and up on a scratch database.
- **Definition.** `WorkflowDefinitionInput.documents` (`WorkflowDocumentInput{item_key, stage_key, template_id, title?, due_offset_days, assignee_role}`, ≤50, keys unique and distinct from checklist keys, stage must exist). `definition_payload` and `stored_definition_payload` include `documents` **only when present**, so every approved version's signed digest is unchanged; the create, version, approve and list routes persist and return them. A synthesized amendment (`workflow_synthesis`) does not carry documents; a firm adds them on the next version.
- **Preview** (`build_preview`): `preview["documents"]` (again only when the version requests any) resolves each request against the firm's document templates (`services/workflow_document_requests.preview_entries`): title, published version number, due date, assignee, and `available` with an `unavailable_reason` for a missing, archived or unpublished template. An unavailable document does not block the run; an attorney-of-record assignee that is unset does, like a task. The published version number is in the preview, so a template published again between preview and apply is a changed preview (409).
- **Apply** (`apply_run`): after the tasks, each available document goes through `propose_document`: `prepare_fill` for the template's published view with the approver as actor (one memoized `Loaders` bundle), `fill_sessions.open_prepared_session` (owner = the task's assignee when there is one, else the approver; answers encrypted as any session), and a `Prepare <title>` task (`source="workflow"`, `external_ref="workflow:<run>:<item_key>"`, description with the Smart Fill counts and the Prepare link `?template=&matter=&session=`). The `document_propose` step records the task fields rollback compares plus `document_template_id`, `template_version_no`, `fill_session_id`, `fields`, `filled`, `missing_required` (names), `sources_loaded` and `prepare_path`: counts and names, never a value. An unavailable template yields a `blocked` step with the reason. The `applied` event detail gains `document_count`.
- **Rollback** (`rollback_run`): `document_propose` steps join the `task_create` steps: the same unchanged-task check, plus a session that is `saving` or `saved` is a blocker ("document <key> was already saved from its session"); on rollback the task is cancelled and the session abandoned (`task_cancel` evidence carries `fill_session_id` and `fill_session_abandoned`).
- **The actor question** (audit §2 "needs a product decision"): documents are prepared at apply time by the person approving the run, not at plan time by the rule's creator. Automation rules therefore reach documents through the workflow template they plan; nothing is prepared until a person approves the run.
- **Fix.** The blocked-rollback `409` embedded `run_response` (UUIDs, datetimes) in the exception detail and could not be encoded; it now goes through `jsonable_encoder`.
- **Frontend.** Workflow settings: a "Documents to prepare" editor (published document templates only, title, stage, due offset, assignee, remove) on the template form, carried into the next version; the versions list shows them. The matter workflow panel's preview lists "Documents to prepare" with the version and whether each can be prepared.
- Tests: `test_workflow_document_requests_postgres.py` (author and approve with documents; preview and apply open the session with the matter's values and the Prepare task, evidence holds no value, the assignee lists and resumes the session; an archived template is reported and blocks only its step; rollback cancels the task and abandons the session; a saved session blocks rollback with a 409; 422 for a bad stage or a repeated key; digest stability without documents; migration text); existing workflow suites unchanged (115); vitest settings and panel suites with the new mock.

## 2026.09.20.04 — Fill sessions, background packet save, and persisted verification (Phases 3d, 4c, 4d)

- **Migration `198_document_fill_sessions`** (head; `down_revision = 197_document_evidence`; head pins updated): `document_fill_sessions(tenant_id, user_id, matter_id, template_id | set_id, title, versions_json, answers_ciphertext, answers_sha256, verified_json, members_json, status open|saving|saved|failed|abandoned, job_id, last_error, expires_at)` with RLS enabled and forced, a check that a session is for exactly one template or one set, and indexes by matter and by user. Applied up, down and up on a scratch database.
- **`services/fill_sessions.py`**: answers are stored with the token-vault `encrypt_token` (an HMAC-only session would discard what a resumable session exists to keep) and decrypted only for the owner's single-session read; `upsert` (create, or update the caller's own; a session that is saving is not edited), `abandon`, `list_for_matter` (answers withheld), `verified_counts` (names only), `enqueue_render` (the `template_set_render` durable job with each previewed member's variables and preview id), and `run_set_render_job`, which saves every member **by calling the render endpoint as the session's user** with that user's own preview evidence, so the evidence gate, staging and matter event are exactly what a browser save gets; a member whose evidence no longer binds is reported as `preview_expired` ("Preview expired; review it again.") and the session goes back to open; a user who lost `manage_documents` blocks the job rather than saving as nobody. Registered in `durable_job_handlers.py` as a `session_row` handler. Departure from the plan: the render handler is not extracted into a service; calling it directly keeps behaviour identical by construction.
- **Routes** (`routers/fill_sessions.py`): `POST /api/fill-sessions` (upsert), `GET /api/fill-sessions/{id}` (own, answers included), `DELETE` (abandon), `POST /api/fill-sessions/{id}/render` (queue the background save), `GET /api/matters/{id}/fill-sessions` (own sessions in progress, answers withheld). All require `manage_documents`.
- **4d.** `prepare_matter_documents` adds `verified` per template from `verified_counts`; `DocumentPrefillTemplate.verified`; the Case Documents banner shows "· N verified".
- **Frontend.** `?session=` on the Prepare route (`prepareRouting.buildPrepareTarget/readPrepareQuery`). The set route (`usePrepareSet`) creates a session on the first answer and updates it 800 ms after each change (answers, verified names, matter, member versions), resumes from `?session=`, and gains **Save all in the background**: it persists the session, queues the saves from the reviewed previews, shows "Saving in the background" only once queued, and polls the session every 3 s until it settles, mapping each member's outcome onto the packet rows (an expired preview goes back to "Not previewed yet"). The single-template route keeps and resumes `variables`, verified names and the matter the same way. `components/documents/FillSessionsList.jsx` on the Documents tab lists documents in progress with Resume (to the Prepare route with the session) and Discard.
- Tests: `test_fill_sessions_postgres.py` (vault round trip and digest; create, resume, update, list without answers, ownership, subject check, abandon; verified counts; the background job saving one member and reporting an expired preview on another, then blocking when the actor lost the capability; migration text), head pins; vitest routing, `FillSessionsList`, the single-template keep-and-resume case, the set background save with polling.

## 2026.09.20.03 — Template sets: fan-out endpoint, sets library, and the Prepare route for a packet (Phase 3c)

- **`POST /api/template-sets/{id}/documents-variables`** (`routers/template_sets.py`, `schemas/document_template_set.py`): `{matter_id?, answers}` → `{documents: {template_id: {field: value}}, unanswered_required, unavailable, resolved_versions}` from the pure `build_interview`, `answers_for_documents` and `unanswered_required`. Every available member is listed even with no answered field; an unavailable member carries its reason and no variables. The merge rule stays server-owned.
- **`tests/test_template_sets_rls.py`**: the runtime-role proof for `document_template_sets` and `document_template_set_items` under FORCE RLS (fail closed with no tenant, tenant A sees only its rows, a write into tenant B's set is refused), the gap the Clio-parity execution status named first. Route tests cover the fan-out, an empty member and a missing set.
- **Sets library** (`pages/TemplateSetsPage.jsx` at `/templates/sets`, linked from the Studio library header): list, create and edit (name, description, module, jurisdiction, ordered members from published templates, per-member follow-published or pin), delete, and **Prepare on a matter** → `/templates/prepare?set=`.
- **Prepare route for a set** (`components/prepare/usePrepareSet.js`, `PrepareSetBody.jsx`, the set branch of `TemplatePreparePage.jsx`): loads the set and each available member's release, the interview for the chosen matter (suggestions fill unanswered questions; the Smart Fill button refreshes and keeps typed answers), groups questions by card with "Appears in N documents", the same progress bar, filters and Verified ticks as one document (`interviewReview` in `templateFillReview.js` returns `fillReview`'s shape). **Generate all** asks the server for each member's variables and previews members three at a time through `renderTemplateFile` (PDF, and Word with signature fields as PDF) or `renderTemplate` (text), with per-member status and **Retry failed previews**; an answer edit invalidates every preview. **Save all to matter** runs `renderTemplate` per ready member in set order, one at a time, with per-member Saved / Save failed and **Retry failed saves**; an evidence-mismatch 409 sends that member back to preview. Verified question names are mapped to each member's field names on its save. Then one Send card per saved PDF with signing roles, and a link to the matter's documents. Unavailable members are shown and skipped.
- Save-all stays browser-driven and sequential (preview evidence is per user); a durable save-all is 3d.
- Tests: route and RLS tests; vitest `interviewReview`, the sets page (list, prepare link, create from published templates in order with a pin), and the Prepare page set case (interview loads with an unavailable member shown, one suggested answer reaches both documents, generate-all with one failure retried alone, save-all sequential with one failure retried alone, verified names on the save, the Send card for the signable PDF).

## 2026.09.20.02 — Documents as a Smart Fill source and the vision fallback (Phases 5d, 5c-2)

- **`DocumentEvidenceSource` (5d).** `template_fill_loaders.load_document_evidence` reads the matter's verified, non-generated documents (newest 50) and their `document_text_extractions` rows, runs `matter_fact_extraction.extract_candidates` over each with the standard fact targets (custom-field targets excluded: accepted custom values already reach the fill), keeps one value per alias (a document with two different answers for one field is a conflict, not evidence; bare "Client:" / "Matter:" labels no longer match every field of that entity), and returns `DocumentEvidence` records. The engine's `FillRecords.document_evidence`, `DocumentEvidenceSource` registered last in `SOURCES` (so first-writer-wins makes it the lowest precedence), aliases derived from `intake_writeback.FIELD_TARGETS` (all already declared by record sources, so `vocabulary()` is unchanged), candidates capped at 0.75 and `review_required` with `provenance.source_document_id/source_filename/source_kind/ocr_confidence`. `prepare_fill` loads it only when a field could take one of its aliases. The router shim exposes `_load_document_evidence` for tests; the quirk campaign runner and the engine tests supply stand-ins; the loader returns nothing for an in-memory stand-in.
- **Prefill follows documents.** `matter_facts_digest` includes document evidence, so a new scan marks readiness stale; `prepare_matter_documents` runs every template through one memoized `Loaders` bundle per matter (`template_fill_loaders.memoized`), so parties, retainer, estate and evidence are read once per run; the `matter_fact_extraction` job enqueues a `document_extracted` prefill when a document yielded candidates.
- **Frontend.** `suggestionOriginLabel` says "From <file> in this matter's documents · check it against the page" and `suggestionConfidenceLabel` shows "Read from a document · N% OCR confidence"; the existing source-document link and Confirm control apply.
- **Vision fallback (5c-2).** `INTAKE_EXTRACTION_VISION_MODEL` (empty by default, so off) and `INTAKE_EXTRACTION_VISION_MAX_FIELDS` (40). `intake_extraction_ai.read_field_clip` sends one clip and its label to the vision model as an image content part with a closed `{value}` schema, meters one `UsageRecord` per clip on the `intake-extraction-vision` route, and returns the value or `None`. `template_form_reading.read_scan(keep_clips_below=, max_clips=)` retains a ≤600 px clip only for readings under the floor when asked; `matter_form_reading.read_against_form(use_ai=)` sends those clips when the firm has `intake_fact_extraction.ai_enabled`, proposes the value as `ocr_field_vision` at 0.6, and says how many it read. `POST .../facts/from-form` takes `use_ai`; the fact review list gains the "Use AI for unreadable fields" checkbox.
- Tests: `test_template_fill_evidence_postgres.py` (fills a blank, never a record value; generated documents, other tenants and conflicts excluded; newest wins and the digest moves; memoized loaders), `TestDocumentEvidence` in `test_template_fill_engine.py`, `read_field_clip` unit tests, clip retention in `test_template_form_reading.py`, the vision route case in `test_matter_document_form_reading_postgres.py`, vitest labels and the opt-in checkbox.

## 2026.09.20.01 — Verification pass and matter documents as evidence (Phases 4a/4b, 5a/5b/5c)

- **Verification in Populate (4a).** `templateFillReview.fillReview(..., verified)` reports `verified` per row and a `verified` count with the `unverified` rows; `usePrepareFill` keeps `verifiedNames` (reset with the review state on matter or template change; a typed value marks its field verified; a Smart Fill refresh that changes a value clears it), `toggleVerified`, `verifyAndAdvance` (verify, then focus the next filled unverified row's control) and an `unverified` filter; `PrepareDocumentBody` renders a `Verified: <label>` checkbox on every filled row, and Enter on the checkbox, a single-line input or a select verifies and advances; `TemplateFillProgress` shows "N of M verified" and an `Unverified (N)` filter. Verification is advisory: Save never waits on it.
- **Verified names travel with the document (4b).** `DocumentTemplateRenderRequest.verified_fields` (≤400 names, each a key of `variables`, deduplicated; 422 otherwise). The render handler records `verified_fields` and `verified_count` beside `filled_variables` on the `document_generated` event, sets the new `matter_documents.generation_summary` (`template_id`, `template_title`, `template_version_no`, `total`, `filled`, `verified`, `verified_fields`) and returns it on the render response, first save and idempotent replay; `MatterDocumentResponse.generation_summary`; the Documents tab preview shows "12 of 19 fields verified when generated · from <template> vN". The hook sends `verified_fields` only when non-empty.
- **OCR reaches matter documents (5a).** `matter_fact_extraction` reads text through the new cache (below), accepts `.png/.jpg/.jpeg/.tif/.tiff` sources, attributes label lines read by OCR as `source_kind="ocr"` at the page's confidence, adds `_ocr_line_candidates` over the recorded detections (a label and its handwritten value merged by the OCR row joiner, at the weaker of the two scores, locator `ocr:<page>:<n>`), and says "OCR found no readable text in this scan." only when OCR ran and found nothing. `accept()` re-proves through the same path. `MatterDocumentFacts` shows "From the scan · N% OCR confidence" and lists the proposal's warnings when candidates exist.
- **Extraction cache (5b).** Migration `197_document_evidence.py` (`down_revision = 196_mcp_usage_idempotency`): table `document_text_extractions` keyed `(tenant_id, document_sha256, engine_version)` with the engine (`text_layer|ocr_local|ocr_azure|mixed`), text, OCR lines, page count, confidence and truncation, RLS enabled and forced with the NULLIF `tenant_isolation` policy; plus `matter_documents.generation_summary`. Model `DocumentTextExtraction`. `services/document_text_cache.py`: `extract()` (text layer; `template_intake._needs_pdf_ocr` → `template_ocr.ocr_pdf` merged per page with `_merge_pdf_text_and_ocr`; images via `image_to_pdf`; an OCR failure or missing runtime is a warning and the text layer stands) and `get_or_extract()` (digest lookup, extract in a thread, insert in a savepoint, re-read on a race, commit and restore the tenant context). `get_matter_document_text` (MCP) reads the cache first and never writes it. No sweep job: the upload job fills the cache.
- **Template-anchored reading of a hand-filled form (5c, v1).** `services/template_form_reading.py`: `field_windows(schema)` (fillable fields with `pdf_overlay` or discovered `page`/`rect`; signing and `value_from` fields skipped; ≤200), `read_scan(scan_pdf, windows, template_page_sizes=, ocr=, thumbnails=)` renders each page with pypdfium2 at scale 2, maps every window from template points to scan pixels by page-size ratio (alignment `scaled`; deskew is a follow-up), crops with a 4 pt margin, OCRs each crop, and keeps ≤200 px PNG thumbnails for the first 60; images go through `image_to_pdf`. `services/matter_form_reading.py`: `form_sources()` (the templates this matter's `document_generated` events name, newest first) and `read_against_form()` (the published version, or a pinned version whose source matches; readings mapped to fact targets by canonical binding or normalised name/label; matched readings become `ocr_field` candidates in the `propose` shape, so `facts/accept` is unchanged; unmatched readings returned for display). Routes `GET .../documents/{id}/facts/form-sources` and `POST .../facts/from-form` `{template_id, version_no?}` in `routers/matter_documents.py`. `MatterDocumentFacts` gains a "Printed form" picker and "Read against the printed form", with the handwriting clip beside each value. Vision fallback on a crop is deferred (5c-2).
- Tests: `test_document_text_cache.py`, `test_template_form_reading.py`, `test_matter_document_form_reading_postgres.py` (new); `test_matter_fact_extraction_unit.py` and `_postgres.py` (OCR candidates, PNG source, cache hit, empty scan, unsupported type); `test_document_templates.py` descriptor test (verified fields on the event, the row, the list and the response; 422 for an unknown name); migration head literals; vitest `templateFillReview.test.js`, the Prepare page verification flow and save payload, `MatterDocumentFacts` OCR confidence and form read, `MatterDocumentsTab` summary line; the Generate dialog's schema test now expects the typed fields as verified.

## 2026.09.19.04 — Send for signature from the Prepare route (Phase 3b)

- `schemas/document_template.py`: `DocumentTemplateRenderResponse` gains `signing_roles`, `signing_placement_required`, `positioned_fields` and `signing_placement_problems` (empty unless a document was saved to a matter). `routers/document_templates.py` fills them from the placement report the `MatterDocument` row is written with, and from the row itself on an idempotent replay, so the caller that just generated the document can offer "Send for signature" without listing the matter's documents again. Asserted in `test_generated_pdf_persists_positioned_signing_descriptor_and_lists_it`, first save and replay.
- `frontend/src/components/signatures/` (new): `signatureRequestRules.js` (the role options, field-origin and status wording, `signatureSendNotice`, and the pure request rules: `signatureRequestProblem` in the order staff can fix it, `buildSignatureRequestPayload`, `placementRolesFor`, `roleOptionsFor`, `freeSignerRole`, `duplicateSignerRoles`, `parseReminderDays`); `useSignatureRequestDraft.js` (signers, dates, placement review of the final PDF, create, open draft, send with `acknowledge_review`, discard; placement state resets when the document changes); `SignatureSendCard.jsx` (the new-request form and the "Where each signer will sign" section, with a `picker` slot for whatever chooses the document); `signerPrefill.js` (`initialSignersForMatter`: the client role from the matter's name and email, every other required role as an empty row, client first).
- `pages/MatterDetailPage.jsx`: `SignatureRequestsPanel` keeps its document select, prepared-PDF upload, queue and review actions, and renders `SignatureSendCard` for the rest; `signatureSendNotice` stays exported from the page. Every accessible name is unchanged and `MatterSigningPlacements.test.jsx` passes as it was.
- `components/prepare/SendStep.jsx` (new): after a save whose response is a PDF with signing roles or fields, the Prepare page stays and shows the card with the matter's client prefilled (`getMatterV2`), a link to the saved document for later, and a "Sent for signature" confirmation with the delivery notice. Anything else (Word, markdown, a PDF with no signature field) lands on the matter as before. `PrepareStepper` gains a sixth step, Send, only for a template with signing fields; it reads "Needs PDF output to send for signature" until PDF output is chosen.
- `components/prepare/usePrepareFill.js`: a Word template with signature fields defaults `convertDocxToPdf` on (`templateHasSigningFields`), and `PrepareDocumentBody` says why under the output-format choice, in both directions. The toggle stays.
- Tests: `signatureRequestRules.test.js` (rule order, payload dates, distinct roles and template-declared roles, delivery notice, prefill), `PrepareStepper.test.jsx` (Send step states), `TemplatePreparePage.test.jsx` (save, stay, prefilled client, request created with the saved descriptor, sent; DOCX defaults to PDF with the reason and the Word choice explained).
- Docs: `docs/template-prepare-route-plan-2026-09-19.md` marks 3b shipped with its two departures, and adds Phase 4 (the click-through verification pass) and Phase 5 (matter documents as evidence: OCR for matter documents, an extraction cache, template-anchored reading of a printed and hand-filled form, a document `FillSource`, an index later), with the survey of what already exists.

## 2026.09.19.03 — Prepare route: Studio to matter in one guided page (Phase 3a)

- `frontend/src/components/prepare/` (new): `usePrepareFill.js` holds the Generate dialog's state machine moved verbatim (values, sources, review confirmations, the once-per-template-and-matter Smart Fill, preview evidence, save); `PrepareDocumentBody.jsx` is its two-pane JSX with a `layout` prop (`modal` keeps the dialog's scroll caps); `MatterPicker.jsx` and `prepareHelpers.js` (`getTemplateVariables`, `friendlyVariableLabel`, `formatMatterLabel`, `getErrorMessage`, `downloadRenderedText`) move out of the page. `RenderModal` in `pages/TemplatesPage.jsx` is now a thin dialog around those pieces; every control keeps its accessible name and its 25 tests and three consumers are unchanged.
- `pages/TemplatePreparePage.jsx` (new) at `/templates/prepare?template=&matter=&folder=&return=` (`App.jsx`, Template Studio shell): loads the published release the way `openRender` does (re-fetches `published: true` when the workspace holds a newer draft), lists matters, and renders `PrepareStepper` (Template, Matter, Populate, Review, Save; states and counts derived from the hook's `fillReview` so the rail, the progress bar and the Save gate agree) above the shared body. A draft opens in the dialog's "Draft preview" mode with Save disabled. On save it navigates to the matter's Documents tab with the document opened, or to `return=` plus the document id.
- `components/prepare/prepareRouting.js`: `buildPrepareTarget`, `readPrepareQuery`, `buildSavedTarget`, `safeReturnPath`. Only UUIDs are carried; a return target must be a same-app path.
- Entry points: `TemplateStudioWorkspace` gains **Use on a matter** (`onUseOnMatter`), enabled for a published template and titled "Publish a tested version first." otherwise; the library row gains **Prepare on a matter** for published templates; the Case Documents readiness banner's **Review and save** goes to the route with template, matter, folder and return preselected (`MatterDocumentsTab.onPrepareTemplate`, supplied by `MatterDetailPage`; without it the dialog still opens); the Probate tab's **Generate filled packet** navigates to the route instead of mounting the dialog.
- `MatterDocumentsTab`: a one-shot `?document=` opens that document's preview after the list loads and is removed from the address, the way `CaseSetupCard` treats `?paperwork=`.
- Tests: `prepareRouting.test.js`, `PrepareStepper.test.jsx`, `TemplatePreparePage.test.jsx` (published-release rule, preselected matter fills once and saves to the documents tab, return path and folder honoured, draft mode, missing/failed template), Studio button states, Probate navigation, documents-tab preselect and hand-off.
- Not in this change: the Send-for-signature step (3b) and sets (3c), per `docs/template-prepare-route-plan-2026-09-19.md`.

## 2026.09.19.02 — Document prefill: fill every published template when a matter has data

- `app/services/document_prefill.py` (new): the `document_prefill` durable job. `enqueue_document_prefill(db, tenant_id=, matter_id=, trigger_event=, actor_user_id=)` runs in the caller's transaction inside a savepoint (a failure is logged and released, never blocking the save), keyed `matter_id:facts_sha256` where `matter_facts_digest()` hashes the engine's own candidate index (`template_fill_engine.collect` over matter, parties, retainer, estate) so unchanged facts are a no-op and changed facts queue a fresh run. `run_prefill_job` reloads the matter, selects active published templates that pass `template_automation_ready` (PDF: approved) ranked by `template_rank` (both now public names in `matter_workspace_capabilities`), caps at 20, runs `prepare_fill()` per published version, and records one `MatterEvent(event_type="document_prefill_ready", note_type="system")` whose `metadata_json` carries per-template counts (`fields`, `filled`, `percent`, `missing_required`, `review`, coverage split, sources loaded, collision aliases) and field *names* only. No value, no rendered page, no `MatterDocument`, no preview evidence. `latest_readiness()` returns the newest record with `stale` = the digest no longer matches the live matter. Registered in `durable_job_handlers.py` as a `session_row` handler.
- Triggers: `POST /api/matters` (beside `enqueue_matter_event`), lead conversion in `routers/intake.py` (which now also fires `matter_created` through `enqueue_matter_event`, closing the gap where the most important "data now exists" moment emitted nothing), the portal questionnaire submit in `routers/matter_intake.py` (`intake_submitted`), and accepted write-back proposals (`intake_writeback_accepted`).
- `GET /api/matters/{matter_id}/document-prefill` (`routers/matters.py`, `schemas/document_prefill.py`): the readiness record or `null`; tenant-scoped through `_get_matter_or_404`.
- Frontend: `components/documents/PreparedDocumentsBanner.jsx` on Case Documents ("N documents ready to review, X% filled from this matter", per-template counts, a stale note) with **Review and save** opening `MatterTemplatePicker` with the new `initialTemplateId` prop, which skips the library step and lands in `RenderModal` where the values are computed live. `api.getMatterDocumentPrefill`.
- Tests: `tests/test_document_prefill.py` (enqueue on create and on conversion, idempotent on identical facts and fresh on changed facts, savepoint containment, the job through the real worker, counts-only metadata with values asserted absent, no document/preview rows, blocked on a missing matter, honest empty run, readiness route with staleness and tenant scoping); Vitest for the banner and the picker preselect.
- Not in this change: rules cannot yet request a document (Stack A `document_propose` step), and Studio has no guided Select → Populate → Review → Send route; both are logged as follow-ups in `docs/smart-fill-engine.md`.

## 2026.09.19.01 — Smart Fill engine: reachable records, every party role, fitted values, and a quirk campaign

- `app/services/template_fill_engine.py` (new): the matter-data resolver, moved out of the templates router. Each record family is a `FillSource` (`current_user`, `estate`, `matter`, `retainer`, `matter_party`, `contact`, `inferred_caption`, `attorney`) that declares the aliases it can produce, whether a template needs it loaded, and how its record becomes candidates; `SOURCES` is the ordered registry and the first writer of an alias still wins, but the losing writes are kept as `Collision` records. `vocabulary()` is the union of the declarations plus `NAME_SYNONYMS`; `tests/test_template_fill_engine.py` proves the source part equals what the resolver writes over a fully populated probe (`tests/fill_campaign/probe.py`, the matter the router used to fabricate at approval time). `prepare_fill()` returns suggestions, renderer-ready `values`, the coverage split, `missing_required` and `sources_loaded` for a template and a matter without a request or user, so a job can prepare documents when a matter has data. `resolve_variables()` keeps the precedence firm profile > custom field > declared binding > name match > nothing.
- `app/services/template_fill_loaders.py` (new): the four tenant-scoped reads (`load_matter_context`, `load_matter_parties`, `load_estate_for_matter`, `load_current_retainer`) and a `Loaders` bundle so callers can replace them as a unit. A malformed or foreign matter id raises `MatterLookupError`; the router maps it to the 422/404 it always returned.
- `routers/document_templates.py`: `build_variable_suggestions` is a shim over `prepare_fill` that resolves `_load_*` on the module at call time (existing monkeypatches keep working); `_collect_smart_fill_candidates`, `_bound_suggestion`, `_smart_fill_alias_vocabulary`, `_stringify_suggestion`, `_add_candidate`, `_caption_parties` and `extract_*_variables` remain as aliases. About 560 lines leave the router.
- Behaviour change: a source loads when a *field name* matches one of its aliases, not only when a binding does. The estate used to load only for an `estate.` binding, so an unbound field named `estate_decedent_name` was classed `name_matched` by `template_fill_coverage`, passed the approval gate, and rendered blank.
- Reachable records: `MatterSource` adds `matter_number`, `practice_area`, `opened_on`; `ClientContactSource` adds `client_first_name`, `client_last_name`, `client_preferred_name`, `client_organization_name`, `client_entity_type`, `client_number`. `template_bindings._CATALOGUE` and `template_cards` gain the matching `matter.number`, `matter.practice_area`, `matter.opened_on`, `client.first_name` … `client.client_number` paths and card fields (additive; every pre-existing path resolves through the same alias, `TestLegacyCompatibility` unchanged).
- Party roles: `CaptionPartySource` iterates `PARTY_ROLES` (plaintiff, defendant, petitioner, respondent, opposing_party, counsel, witness, expert) instead of the civil pair; `template_cards` gains a role card and `template_bindings` a `party.<role>.name(s)` pair per new role; `schemas/matter_party.py` lists their `template_fields`. `client` is left out (the client contact owns `client_*`) and `other` names nothing. `matter.role` inference still covers plaintiff/defendant only.
- `NAME_SYNONYMS`: on the name-match branch only, `first_name`, `last_name`, `full_name`, `name`, `email`, `phone`, `address`, `city`, `state`, `zip`, `dob`, `company` and a few spellings resolve to the client's alias at confidence 0.9, `review_required`, with `provenance.synonym_of`. A binding is never second-guessed.
- `app/services/template_fill_formatters.py` (new): `format_suggestion` shapes a value to the field's type and records `formatted_from`/`format_note`, or withdraws it with `format_warning`/`unformatted_value`. Choice/radio: an option is chosen by export value or label, case-insensitively, and a US state by code or name either way (`ND` selects "North Dakota"); checkbox: `true`/`false` from yes/no/x/1; date: ISO to `MM/DD/YYYY`. Text fields are untouched, so `250.00` stays as the template author sees it. Applied in `prepare_fill`, so the preview route and the set interview both return fitted values.
- Quirk campaign (`tests/fill_campaign/`, `scripts/rehearse_smart_fill.py`): six mock matters (person client, entity client, family petitioner/respondent, probate estate, bare matter, two defendants), an AcroForm PDF and a Word letter whose fields are named in every convention a firm's form uses, plus the five seeded sample forms that ship bindings; each case resolves, renders through `fill_pdf_template`/`fill_docx_template`, reads the page back and reports `filled | blank | choice_mismatch | signature | dropped_by_renderer` per field with source, coverage state and note, plus alias collisions. `tests/test_fill_campaign.py` pins the outcomes as fences (the ones this change fixed flipped in the same commit), `tests/test_fill_campaign_seed_pdfs.py` pins that every bound field of the five forms fills (the fee agreement's `contingency_percentage` is the one legitimate blank for an hourly matter), and `tests/test_smart_fill_route_matter.py` resolves persisted rows through `/smart-fill-preview`. Over the campaign, filled outcomes rose from 447 to 538 of 1,261 with no render errors.
- Still pinned as quirks: two PDF widgets whose names fold to one variable (`client_name` and `Client Name`) get a `_2` suffix and the second no longer matches by name; `ClientName` matches nothing; a second defendant is reachable only through `defendant.2.full_name` (its alias `defendant_2_name` is producible but not in the approval vocabulary); a structured party silently wins over `matter.counterparty` (now reported as a collision); `Decimal` money renders raw in text fields.
## 2026.09.19.05 — Assistant drafts keep their formatting, and can update tasks

- `app/services/cloud_artifact_materialization.py`: `render_revision_docx` now parses the Markdown that external MCP clients actually emit instead of emitting one bare paragraph per line. Headings become Word heading styles, `**bold**`/`*italic*`/`~~strike~~`/`` `code` `` become styled runs, `-`/`1.` lists use the List Bullet/List Number styles, `>` becomes Quote, fenced code is rendered in Consolas, pipe tables become real Word tables, and blank lines are skipped rather than materialized as empty paragraphs. Before this, a motion drafted in ChatGPT or Claude arrived in the matter as literal `##` and `**` text for a paralegal to reformat by hand. Tests: `test_renderer_parses_markdown_structure_and_inline_emphasis` plus the control-character and paragraph tests in `test_cloud_artifact_materialization.py`.
- `app/services/automation_capabilities.py`: `CapabilitySpec.parse_arguments` now appends the actionable size to a length rejection via `_argument_error_detail` — for example `received an invalid body (value has 61240 characters; the limit is 50000)` — so a model can correct an oversize call instead of retrying blindly. Only lengths are echoed; the offending value is never copied into the error. The `propose_matter_document`/`propose_client_email` descriptions now state the 50,000-character body, 20,000-character email body, and 10-source caps. Test: `test_invalid_argument_errors_name_the_submitted_size_and_limit`.
- `app/services/mcp_product.py`: `usage_summary` (and the pure `_summarize_usage_rows`) now separates `billable_calls`, `unbilled_calls`, and `failed_calls`, and adds a per-`auth_type` breakdown. Internal chat reads the same corpus but is never invoiced, so blending it into `total_calls` made a tenant's billable volume impossible to reconcile against the Stripe meter. Test: `test_mcp_usage_summary.py`.
- Workspace MCP gains `propose_task_update` (workspace-only): `app/schemas/workspace_mcp.py` (`ProposeTaskUpdateArgs`, at least one change required), `app/schemas/chat_action.py` (`TaskUpdateAction` in the `PendingAction` union), `app/services/chat_tools/handlers.py` (`propose_task_update` stages a Review task), `app/services/task_automation.py` (`_run_task_update` applies it after approval, re-locking the target task and re-validating tenant/matter/assignee; a reason is required for Waiting/Cancelled), and `app/services/workspace_mcp_protocol.py` (RBAC map). It reuses the existing `matters:read` + `tasks:propose` scopes and `manage_matters` RBAC; no new scope, migration, or raw write. Tests: `test_mcp_task_update_unit.py`, updated catalog assertions in `test_automation_capabilities.py` and the protocol tool-list tests.
- Documentation: new `docs/reviews/MCP_AUDIT_REVIEW.md` records the Workspace/Research MCP audit, the coverage gaps, the RAG query/metering findings, and this review's fixes; `docs/workspace_mcp_adapter.md` documents `propose_task_update`.
## 2026.09.18.01 — North Dakota probate: intake, track determination, pre-filled court forms

- `app/services/probate/` (new): `facts.py` (`ProbateFacts`, tolerant parsers for dates, money, yes/no, heirs tables, JSON round trip, gap-filling `merge`), `determination.py` (pure `determine()` over the facts: `informal_testate`, `informal_intestate`, `formal_testate_late`, `formal_intestate_late`, `small_estate_affidavit`, `not_nd_domicile`, `undetermined`, each reason citing N.D.C.C. Title 30.1; five-day wait, three-year bar, $100k small-estate ceiling, venue by domicile or property, waiver of priority), `deadlines.py` (rules table anchored on death / appointment / first publication / closing; `sync()` upserts `estate_deadlines` by type and optionally mirrors `tasks` by `external_ref`), `forms.py` (NDPC Forms 1–19 registry, `page_ranges()` read from the guidebook's headers, `GUIDEBOOK_BINDINGS` field map, `forms_state()`), `bindings.py` (`estate_candidates()` for Smart Fill: caption, decedent, heirs table, applicant/PR blocks, statements, dates, Form 10 inventory totals from verified assets), `intake.py` (portal and returned-PDF answers → facts; `preview()` for the reviewer's task), `install.py` (idempotent copy of the shipped pack into the tenant).
- `app/services/practice_resolution.py`: `PackQuestion` gains `kind`, `options`, `help`; new `probate` practice (`PROBATE_QUESTIONS`, `PROBATE_UPLOADS`) takes the `probate` alias from `estate` and joins `_SCOPE_ALIASES`. `app/schemas/matter_intake.py`: `IntakeQuestion.kind/options/help`; `matter_intake.validate_answers()` checks typed answers (date, money/number, yes/no, select) and replaces the inline loop in the portal submit route. `intake_writeback.plan_writeback` stores a `probate` preview and names the track in the review task.
- `app/services/pdf_templates.py`: `MAX_PDF_WIDGETS = 400` (was 200) so the court's 63-page packet stays one template; matching request-schema caps; `read_pdf_form_values` reports a radio group's chosen export value instead of `"true"`.
- `app/services/template_bindings.py`: `estate.*` group (47 paths); `template_cards.py`: an `estate` card derived from the catalogue; `routers/document_templates.py`: `_collect_smart_fill_candidates(..., estate=)`, `_load_estate_for_matter()` when a template binds to `estate.*`, and a populated probe estate in `_smart_fill_alias_vocabulary`.
- `app/services/sample_import.py` (new): `import_sample_as_template()` copies a platform sample into a tenant's templates as a draft, idempotent by `source_provenance.sample_slug` — the first sample→tenant path.
- Migration `195_probate_track`: nullable `estates.domicile_county`, `will_execution_date`, `probate_track`, `probate_facts`, `probate_determination`, `probate_determined_at`, `appointment_date`, `first_publication_date`, `letters_issued_date`, `closing_statement_filed_date`; `estate_assets.source` (default `staff`), `verification_status` (default `verified`), `submitted_at`, `artifact_document_id`.
- `routers/probate.py` (new, `/api/plugins/trust-estate`, `require_addon_workflow("trust-estate-legal")`): `GET/PUT …/probate(/facts)`, `POST …/probate/facts/from-intake`, `POST …/probate/determine`, `PATCH …/probate/anchors`, `POST …/probate/deadlines/sync`, `GET /probate/forms` (installs the pack on first sight), `POST /probate/forms/install`. `routers/estates.py`: `POST …/assets/{id}/verify`; "Domicile county" joins the missing opening facts for probate estates. `routers/client_portal_estate.py` (new, `/api/portal/client/estate…`): fail-closed client inventory, open after appointment or when staff open it.
- `app/schemas/estate.py`: child responses inherit `OrmResponse`, whose before-validator turns `uuid.UUID` into `str`. Every estate sub-entity endpoint that returned an ORM row (`POST /estates/{id}/assets` and its siblings) previously failed response validation under pydantic v2; no test had covered them.
- Seed library: `court_form/nd-informal-probate-guidebook.pdf` (Forms 1–19, 340 fields, 164 bound, `page_ranges` in the manifest) and four companion forms, built by `backend/scripts/build_nd_probate_guidebook.py` (strips `/URI` links, the structure tree, outlines and metadata; clears required bits; sets multiline on heirs/inventory fields; refuses a map that names a missing field or an unknown binding). `intake/nd-probate-intake-questionnaire.pdf` authored from `PROBATE_QUESTIONS` by `build_library_intake_forms.py` (`--only`). `scripts/build_sample_template_library.py` preserves `origin: "court_form"` entries. `seed/sample_templates/README.md` documents `court_form` and `page_ranges`.
- Frontend: `components/estate/ProbateTab.jsx` (determination card, staff facts form, court forms with print pages and `RenderModal` generation, deadline clock, send-intake); `EstateDetailPage` gains the Probate tab, domicile county / will date fields, an `applicant` fiduciary role, the new deadline types, and a client-submitted assets review panel; `CaseSetupCard` opens the paperwork drawer from `?paperwork=`; `ClientIntakeChecklist` renders typed controls, keeps a per-device draft, offers "I don't know"; `IntakeSetupFields` keeps pack keys/kinds while the list is untouched; `components/portal/ClientPortalEstateTab.jsx` and an optional "Estate inventory" portal tab; `api.js` bindings. Add-on retitled "Trust, Estate & Probate".
- `app/services/probate/base.py`, `registry.py`, `jurisdictions/north_dakota.py` (new): the jurisdiction seam. The workbench, Smart Fill bindings and deadline clock resolve a `ProbateJurisdiction` bundle (determine, deadline rules, forms, question set) from the estate's `jurisdiction` field (blank/free text → North Dakota; an explicit unsupported state fails closed with no ND rules or forms). ND is the first registration; a second state is a new module plus a registry row, no change to shared screens or storage. `routers/probate.py` gains `GET /probate/jurisdictions` and a `?jurisdiction=` selector on the forms endpoints; `ProbateStateResponse` reports `jurisdiction`/`jurisdiction_label`/`jurisdiction_supported`.
- Security: every `/api/plugins/trust-estate` staff route (`estates.py` and `probate.py`) now requires the `manage_matters` capability in addition to the add-on entitlement. A portal client login (`role="client"`, which `get_current_user` accepts) can no longer replay its token to read or rewrite estate/probate records; form installation still requires `manage_documents`. Regression test in `test_probate_routes_postgres.py`.
- Tests: `unit/test_probate_determination.py` (17), `unit/test_probate_facts.py`, `unit/test_probate_deadlines.py`, `unit/test_probate_bindings.py`, `unit/test_build_nd_probate_guidebook.py`, `unit/test_probate_jurisdiction.py`, `test_nd_probate_forms.py`, `test_intake_typed_answers.py`, `test_probate_routes_postgres.py`, `test_probate_smart_fill.py`, `test_client_portal_estate.py`, a write-back preview case, and Vitest for `ProbateTab`, `ClientIntakeChecklist`, `ClientPortalEstateTab`, `IntakeSetupFields`, `EstateDetailPage`.

## Unreleased — Chat recovers from an empty model response

- `app/services/llm.py`: a gateway that answers HTTP 200 while streaming only hidden reasoning tokens (no visible content) no longer fails the whole chat turn on the first attempt. `stream_complete` retries the same resolved route once with a doubled output budget so hidden reasoning cannot starve the visible answer, and only then raises the existing "no visible answer" error. LiteLLM's own fallback chain fires on provider errors, never on an empty successful response, so this case had no gateway-level recovery and surfaced to the user as "no response". Bounded to a single retry per route so a genuinely broken model cannot multiply latency or spend. Tests: `test_empty_visible_stream_is_retried_with_a_larger_budget`, `test_empty_visible_stream_fails_after_a_bounded_retry`, and `test_stream_falls_back_to_the_next_candidate_before_first_token` in `test_llm_gateway_stream_contract.py`.
- `app/services/error_tracker.py`: `capture_error` no longer overrides an explicitly supplied severity to satisfy the status-code map. A handled 503 — chat generation backpressure (`chat._GENERATION_CAPACITY_DETAIL`) and the retryable customer-storage outage — is deliberately recorded as "error"; the old auto-map silently promoted it to "critical", so an expected, self-healing condition read as a production incident in the operator error log. Auto-mapping still applies when the caller gives no severity. Tests: `test_capture_error_keeps_an_explicit_severity` and `test_capture_error_auto_maps_severity_when_not_given` in `test_error_tracker.py`.

## 2026.09.18.01 — Notifications say what they are for

- A task reminder used to carry a title, a date and nothing else: no client, no matter, no link, nothing that told the recipient which piece of work it meant. `EmailService.send_task_reminder` now carries the same identifying context as the assignment alert — client, matter, attorney of record, status, priority, type, who raised it and when — with a link straight to the task and a subject that leads with the urgency and the client (`Due today: Follow up on outstanding intake documents — Jane Doe · Doe Estate Administration`). Every interpolated value is escaped; the reminder previously injected titles and matter names into the message HTML raw.
- `app/services/email.py`: shared notification building blocks (`detail_row`, `link_row`, `detail_table`, `cta_button`, `safe_link`, `text_lines`, `labeled_line`) so every LawHand alert answers the same four questions — what, for whom, by when, where to go — and escaping lives in one place. `safe_link` refuses anything but http(s), so a malformed link can never render as a clickable control in a message the recipient cannot inspect.
- `app/services/task_notifications.py`: `send_task_due_reminder` resolves that context once and is now the single path for both the nightly sweep and the manual **Remind** button, which previously built their own thinner payloads. `due_label` turns a date into the words a subject line needs (`Overdue by 3 days`, `Due today`, `Due tomorrow`). A task carrying client SMS content sends a reminder stripped of client, matter, attorney and description: SMS review stays in LawHand, and the assignee still gets the deadline and the link.
- Estate deadline reminders reuse the same email with their own labelling (`Deadline`, the estate as the matter, the deadline type, the notes) and a link to the estate, instead of packing all of it into a synthesized title.
- `app/services/client_notifications.py`: one shape for every notification sent to a client. The **firm's** name leads rather than the platform's, the matter is named, a deadline is stated as a date and a countdown, and the message ends in one button into the portal. No document body, message text or signature page is copied into email, and no link carries a session — the portal lands on sign-in, and the one-time code remains the only way in.
- Signature invitations and reminders are built through it, so a client now learns which firm is asking, for which document, on which matter, and by when. They also reach a page that exists: both the email and the SMS pointed at `/client-portal`, which has never been a route, so every signer who clicked one got the 404 page. They now link to `/portal/client/matter?tab=signatures`.
- Portal nudges (a document shared, a message from the firm) go through the same framework and link to the tab they are about.
- The "every signer has signed" notice back to the firm gets the branded shell, a detail table and a link to the matter, instead of three unwrapped paragraphs.
- Client portal: `?tab=` opens the portal on the tab a notification linked to, the open tab is reflected in the URL so the link stays shareable, and the tab survives the sign-in bounce for a client who was signed out.
- Tests: `test_client_notifications.py` (9) for the client framework, escaping, the portal URL and the firm-identity fallbacks; 11 new cases in `test_task_notifications.py` for the reminder's content, the urgency wording, the resolved context, the SMS withholding and link safety; 4 frontend cases for the deep-linked tab.
...
- `POST /api/matters/{id}/signatures/{request_id}/send` accepts an optional `{"acknowledge_review": true}` body and returns 422, naming the warnings, when the stored plan requires review and none is given. `SignatureRequestResponse` carries `plan_review` and `plan_review_required`; requests planned before this change carry neither and send as before.
- Case Setup (`matter_intake.start_packet`) creates its requests already sent, so a plan with warnings is written to the matter timeline as a `Signature plan needs review` event instead.
- E-Signature panel: submitting the form now *prepares* the request and shows "Where each signer will sign" — every signer's signature, initials and date fields by page, with where each came from (a field in the PDF form, placed by you, a line found on the page, or no line found). Warnings must be ticked as checked before Send is enabled; Discard voids the draft; a draft left behind can be reviewed and sent from the list.
- A generic `signer` role — the one portal signer Case Setup addresses, and the default when a request names none — now reads as the client, so it takes a `Client:` line. Without this, `2026.09.16.05`'s rule that a signer never takes another party's line left every intake fee agreement on the fallback block.
- Tests: `test_esign_plan_review.py` (26) for the review, the stored summary and the send gate; `test_matter_intake_plan_review_postgres.py` (3) for the timeline event; two real-layout cases for the generic signer; 7 panel tests for the review step.
...
- Separate the template's source format from the saved artifact's format when binding. Only a PDF template carries overlay geometry; only a PDF artifact can be reviewed by hand. A Word template converted to PDF is unbindable but recoverable; saved as `.docx` it is neither, and `PlacementReport.recoverable` says which.
- Persist the reasons on `matter_documents.signing_placement_problems` (migration `194_signing_placement_problems`), expose them on `MatterDocumentResponse`, carry them through the folder-copy path, and log them at generation with the template and matter id. Generation previously discarded the reason and emitted nothing.
- `POST /api/matters/{id}/signatures`: build the 422 from the stored reasons via `placement_block_detail`, naming the failing field and its remedy, and detect a non-PDF document so the message offers Word-to-PDF conversion rather than a placement review that cannot open it.
- Gate `POST /api/templates/{id}/publish` and `PATCH /api/templates/{id}` (`is_active: true`) with `_ensure_signing_fields_placeable`, refusing a template whose signing fields have no signer role, or — for a PDF template — were never positioned, with every offending field named.
- `app/services/esign/plan.py`: stop a detected signature box being widened over the label sharing its baseline ("Signature: ____  Date: ____"); the right edge now stops where the next text starts, and a box that already fits is left unchanged.
- `app/services/esign/plan.py`, signature-line detection reworked against four real documents that each came out wrong (a referral fee agreement placed the client on the firm's `By:` line; a 13-page parenting plan offered a parent thirteen child-support blanks to sign; a divorce stipulation's execution blocks were invisible; a cover sheet with nothing to sign): a blank opening a wrapped line of prose is a fill-in, not a signature; a bare rule under a line ending in a colon is that line's answer blank; a margin line number (`[96]`) ahead of a blank is not its label; `Dated:` is a date; a party-captioned blank (`Client: ____`) is a signature line only within reach of a `Dated:` blank; a bare rule takes its party from the caption printed under it (`Notary Public`, `[PLAINTIFF'S FULL NAME], Plaintiff`, the wrapped `NAME] , Plaintiff`), and a captioned name line under a signature (`______________, MOTHER`) labels the rule above rather than becoming a second place to sign; a date pairs with the nearest line within reach whether printed right, left or on the line above. Party vocabulary extended to plaintiff/defendant/petitioner/respondent/mother/father/husband/wife/landlord/tenant/buyer/seller/borrower/guarantor.
- `_deal_detected_lines` replaces `_prune_foreign_lines` + `assign_roles` for detected lines: a captioned line goes to its party and nobody else, an uncaptioned line only to a signer who still has none, and a signer left without a line gets the fallback block. A signer is never placed on a line printed for somebody else; a `client` role sent a two-party stipulation falls back rather than signing as Plaintiff.
- Tests: `tests/esign_real_layouts.py` rebuilds the four documents' execution blocks at their real coordinates with no real names; `test_esign_real_layouts.py` (60) pins each outcome, the never-another-party's-line invariant across all of them, and each rule in isolation. `LAWHAND_SIGNING_CORPUS=<dir>` opts a local folder of real PDFs into a smoke test that bounds how many places one signer may be offered.
- Tests: `test_esign_placement_diagnostics.py` (68), `test_esign_plan_detection.py` (38), `test_signing_placement_gates.py` (24), five end-to-end cases in `test_document_templates.py`, and 19 frontend tests covering `signingPlacementProblems.js` and the E-Signature panel.
...
- Resolve the runtime storage identity through `google_service_account.prefer_service_account()`: matter-folder provisioning, uploads, reads, search, sharing, rename, repair, metadata sync, discovery, and cloud-document delete now use LawHand's service account for an `owner_type=org_shared_drive` tenant, so the turnover guarantee covers LawHand availability, not only custody. The service-account token is used only after a cached probe confirms it can reach that customer's drive; otherwise the delegated token is used, and a failed Shared Drive root creation falls back to the admin's My Drive rather than stranding onboarding. SharePoint access remains delegated until Microsoft app-only lands.
...
- Keep public registration default-off and reversible. If enabled, a registration now creates only an inactive, pending firm and inactive founder: no session, trial clock, payment customer, module access, or AI spend begins until a Platform operator approves it. Production preflight accepts either explicit matched flag state and still rejects drift.
- Add two operator paths for tomorrow's customer: approve a pending registration with a chosen trial duration and optional sponsored Premium AI, or privately create the firm and send the founder a single-use account-setup invitation. Both customer emails are branded, and Platform surfaces delivery plus the private invitation's one-time backup URL.
- Turn the Platform tenant panel into the trial control plane: show every access expiration rather than demo expiry only; set an exact UTC date; extend from the later of now or the current end by 30 days or six calendar months; revoke immediately with a past expiry; or clear the expiry to convert the firm to unbounded active access. Each mutation reloads the authoritative tenant state instead of optimistically confusing `trial_ends_at` with `expires_at`.
- Send a branded extension notice to every active firm administrator after the database commit. Delivery failure is returned as `trial_email_status` and shown to the operator without rolling back the access decision.
- Add `tenants.premium_ai_trial_enabled` in migration 193. It is false by default and is the explicit tenant half of a sponsored-trial grant; Platform toggles it together with every licensed human user's existing flag. New invitees inherit an active sponsored-trial grant. Demo and fixture tenants still fail closed, and a user flag is still required.
- Expose Premium status for the tenant and its users in Platform, audit every grant/revocation and affected-user count, and update tenant-admin/licensing checks to respect only the explicit firm gate rather than rejecting every trial categorically.
...
- Ported into `TemplateStudioEditor` first, so nothing left with the file: the failed-preview path (offer the original, and refuse to place a field whose position cannot be measured — `canPlaceFields` requires signed page geometry where the render failed, where this editor previously armed every tool over a blank div), review-state colouring, and the AI-proposal panel.
- The wizard's document preview is no longer suppressed for PDF. It was, because the retired editor carried its own; without that change, removing it would have left a firm looking at a list of field names with no sight of the document they came off. Caught by the wizard's own tests.
- Drop the create-time "include at least one reusable field" check and the PDF preview-ready gate. Both were right while the wizard was the only place to place a field. Kept, the first would refuse the draft that is now the only route to the editor where fields are placed, leaving a firm holding a flat PDF with nowhere to go; the second required the browser to have rendered a page before a draft could exist. Publish is what needs guarding, and `pdf_source_review` now guards it.
- Two things the live app showed that the tests could not. The "N need review" count ignored the source-review confirmation while the field colouring honoured it, so a confirmed template read "41 need review" over a page of green boxes. And the inspector panel was clipped: a `fieldset`'s UA default is `min-inline-size: min-content`, so the field list's longest label set the panel's content width and the label input and fill-source explanation ran past the 288px track with no scrollbar to reach them.
- Every field a PDF scan produces is born `review_required`, including an AcroForm field read with full confidence, so the banner no longer calls them fields "the scan was unsure of" — the flag means nobody has checked them yet, which is what it now says.
- The coordinate contract asserted through the retired editor moves to `pdfFieldGeometry.test.js`, which is where it belongs now that only one surface consumes it, along with the round-trip-at-any-zoom and viewport-precedence cases the old test did not cover. `useTemplatePdfDocument` is stubbed switchably in the editor's tests, so the "the page did not load" branch is exercised rather than assumed.

## 2026.09.15.08 — PDF source review, enforced

- Add `app/services/pdf_source_review.py` and gate `publish_template` on it. The wizard's "I compared every highlighted field with the original document" checkbox disabled the save button in the browser and was never sent anywhere: nothing stored it, `POST /templates/intake/create` accepted a template without it, and `publish_template` enforced source review only for Word (`_ensure_word_source_review` returns early for every other format). A PDF of OCR guesses could be published and used to generate real documents with nothing having confirmed the boxes sit where the scan claimed.
- The attestation is recorded against a digest of the field set it was made for (`variable_schema.pdf_source_review.confirmed_digest`), which is how the Word review gets its "recomputed from the live source" property without a live source to recompute from. Adding, removing or moving a field, or a rescan changing a confidence, re-arms review; a label or binding edit does not, since neither is what a person checked. `_stamp_pdf_source_review` computes the digest server-side from the schema being saved — a client supplying its own could attest to a field set other than the one it is writing.
- `field_needs_review` is the same rule both editors colour by: `review_required`, confidence below 0.75, or an OCR overlay. A scan whose `detection.method` was OCR counts every field, since the page was read as an image rather than as form fields. An unreadable confidence counts as unconfident. Mirrored in `frontend/src/components/templates/pdfSourceReview.js`, with both suites pinning the same cases — an editor working from a different rule would report a template ready and watch publish refuse it. Deliberately *not* the editors' "N need review" count, which also counts AI proposals: that one is an authoring prompt, this one is a publish gate.
- The confirmation moves from the upload wizard to `TemplateStudioEditor`, where a field that looks wrong can be corrected rather than only attested to. It gates publishing rather than creating a draft, which is the risk that warrants a gate: a draft generates nothing.

## 2026.09.15.07 — Fill coverage, and a binding control on the upload flow

- Add `TemplateBindingPicker` to both intake surfaces — `PrepareFormWorkspace` (PDF and image) and `WordImportWorkspace` (.docx) — beside the existing Label / Type / Required controls. `PrepareFormWorkspace` contained the string `binding` zero times and `TemplateBindingPicker` had exactly one consumer, in `TemplateStudioEditor`, so the screen where a firm first sees its own form with every discovered field highlighted was the one screen that could not say where a field's value comes from. Nothing else was missing: `TemplatesPage` already carried `binding` through intake state and `PATCH /templates/{id}` already validated it against `template_cards.is_valid_path`. This matters most for tenants without premium AI: `/templates/intake/ai-propose` is gated on `require_capability("use_premium_ai")` and `premium_ai_enabled` because the API cost is metered, so the manual path is the default for every other tenant and it was the path with no control.
- Add fill coverage — how many of a template's fields arrive with a value when a matter is named. Both editors reported *discovery* coverage ("N fields · N need review") and neither reported this. Six states, kept apart because they fail differently: `bound` (declared, resolvable), `name_matched` (no binding, name in the Smart Fill vocabulary — works today, stops working silently on a rename), `manual`, `signature`, `unresolved` (declares a path the catalogue can no longer resolve) and `unbound`. Excluded fields and `value_from` copies are out of the denominator, as they are out of the fill review's rows.
- The rule lives in `app/services/template_fill_coverage.py` and is mirrored in `frontend/src/components/templates/fillCoverage.js`, because the read-out has to answer to an author renaming a field or picking a source in a schema nobody has saved yet, while the library list needs the saved truth without loading an editor. The client is not allowed a second copy of the *vocabulary*: `GET /templates/bindings` now serves `smart_fill_names` (the normalised names `_smart_fill_alias_vocabulary()` produces), and both implementations pin `backend/tests/fixtures/fill_coverage_contract.json` to the same split. The Python suite additionally asserts the catalogue slice that fixture hands the JavaScript side still matches the real server.
- `GET /templates` and `GET /templates/{id}` carry `fill_coverage` (`_template_response`), so a firm can tell a well-wired template from a badly-wired one from the library list.
- Fix: a required field bound through the card rail was refused at publish. `_binding_is_resolvable` resolved through `template_bindings.alias_for_binding` (the flat catalogue), while the fill resolves through `template_cards.alias_for_path` (a superset). Card paths are what the card rail actually emits, so `client.full_name` and `defendant.2.full_name` — set with the primary binding control — failed `_validate_approval_ready` with "required field(s) with no data source and no default" although `_bound_suggestion` filled them from `client_name` and `defendant_2_name` without trouble. Both now resolve through the card resolver.
- A "Fill source" highlight recolours the field rectangles by state in both editors, as a toggle rather than a repaint: in `PrepareFormWorkspace` the existing colours carry review status, which is a compliance step, and overloading one colour channel with two questions makes a matrix nobody can read. The Studio strip also gives a PDF template a field count, which it never had — the count lived in the Word-only branch of the toolbar.
- `useBindingCatalogue` moves out of `TemplateStudioEditor.jsx` into its own module and gains `smartFillNames` and `catalogueLoaded`. Coverage is withheld until both catalogue requests land, since against an empty catalogue every bound field reads as broken.
- `_normalize_variable_name` and `_binding_is_resolvable` in `document_templates.py` are now thin aliases of the service's, so the approval gate and the read-out cannot drift on whether a field has a source.
- `GET /templates/queues` and the Studio home cards report fill coverage per template. Suppressed where a template has no fields: "0 of 0" on a template nobody has set up is noise.

Six divergences found by diffing `PrepareFormWorkspace` against `TemplateStudioEditor` (`docs/template-studio-editor-diff.md`). Each is a silent failure — the editor accepts the edit and the product does something else:

- `PrepareFormWorkspace` drew placements at `scale: pdfSample ? 1 : zoom`. `scale` is read only where there is no pdf.js viewport to transform through, which is exactly the external-fallback path, and the page box there is `page.width * zoom` wide — so every field was drawn 1/zoom too large and a drag committed back through `geometryToOverlays` at the same wrong scale. The fallback exists because the page cannot be rendered, so nobody could see it. Now `scale: zoom`, which is what Studio already passed.
- `TemplateStudioEditor` offered `multiline` as a `field_type` and stored it without the `multiline` boolean. `fill_pdf_template` wraps on the boolean (`pdf_templates.py:1560`), so a paragraph re-typed in Studio stopped wrapping, and a paragraph authored at intake — `{ field_type: 'text', multiline: true }`, what `createManualField` writes — read back in Studio as plain text. Both editors now use the one model.
- Studio's Required checkbox and Type select were live for AcroForm fields. `_reviewed_variable_schema` takes `field_type`, `multiline`, `options`, `page`, `rect` from the live PDF and ORs `required` with the source's for non-checkboxes, so both controls promised an edit that reverted on save. Both are now disabled with the reason, as they already were at intake.
- Excluding a field in Studio was a one-way door: the canvas hides an excluded field and the only control wrote `included: false`, with no way back. The inspector gains the "Include in template" checkbox intake has.
- Studio's canvas placements were keyboard-inert — a bare `Rnd` with a `pointer-events-none` label, so a keyboard-only author could not select, move or delete a field on the page at all. They are now focusable buttons with intake's arrow-nudge (1px, 10px with Shift) and Delete/Backspace.
- Four of Studio's six undo push sites snapshotted `{ fields, regions, coverRegions, sourceReview }` and two snapshotted a subset, while `undo` restored `previous.coverRegions || []` over the real ones and the next save persisted the empty list. One `editorSnapshot()` now serves every push site.

## 2026.09.15.06 — Trial conversion, billing-only access after expiry, tenant-level premium AI

- `platform_billing.end_trial_when_paid` clears `Tenant.expires_at`, clears the `trial` marker, drops `trial_started_at`/`trial_ends_at` and records `trial_converted_at` once Helcim reports a subscription that is live, billed (`timesBilled > 0`) and free of failed payments. It runs after every `apply_subscription` path (checkout completion, refresh, signed `cardTransaction` webhook, scheduled reconcile). `apply_subscription` set `billing_tier`/seats but never touched the trial, so a firm that paid on day ten was still refused at `require_active_tenant` on day thirty and still blocked from premium AI. Pending, failed and cancelled subscriptions leave the trial untouched, as does a paid firm that never had one.
- New `app/services/tenant_access.py` holds the access rules. `require_sign_in_tenant` admits an active, non-synthetic firm whose only problem is an elapsed `expires_at`, and is used by `_issue_access_token` (password, OAuth and Office-free paths), the OAuth existing-user branch, `/oauth/exchange` and `/auth/refresh`. `get_current_user` then allows such a firm only `/api/auth/me`, `/api/billing/status` and `/api/billing/subscription/*` (`EXPIRED_TRIAL_ALLOWED_PATHS`/`_PREFIXES`); every other route keeps returning 403 "Tenant access has expired". **This is a deliberate loosening, limited to account and billing surface:** previously the only page that could end the lockout was itself locked. Inactive firms, missing firms and expired demo/fixture workspaces are refused exactly as before, and a refresh for an inactive firm still revokes the family.
- `subscription_offer` rejects a Helcim payment plan carrying `freeTrialPeriod > 0` (503) and pins `trial_days` to 0; `finish_checkout` always sends `withFreeTrialPeriod: false`. LawHand runs the trial from signup, so a provider-side trial would have started a second free period on the day a firm subscribed.
- Premium AI now requires the per-user flag **and** a firm that is not on a trial and not synthetic, enforced where premium runs rather than only at the toggle: `chat._premium_for_user`, both `plugins.py` run paths, the template AI proposal route, `matter_document_revisions`, and the email agent. `resolve_user_premium_ai(db, user)` resolves the firm from `user.tenant_id` when the `tenant` relationship is not loaded — the chat handler builds its user from the verified token — and checks the flag before any lookup. **Tightening:** `/api/email/scan` drafts and `/api/email/draft-response` used the premium model for every user including trial firms and users whose flag was off; they now follow the same rule, and `EmailAgent.draft_response`/`process_emails` take `use_premium`/`premium_drafts`.
- `GET /api/auth/me` reports `access_state` (`active`/`trial`/`trial_expired`), `trial_ends_at` and `premium_ai_available`; demo workspaces report `active` because their expiry is a demo session, not a convertible trial. The browser does not consume these yet.
- Docs: `docs/trial-access.md`.

## 2026.09.15.05 — Staff invitations, OAuth refusal redirects, reset hardening

- Add `user_invitations` (migration `192_user_invitations`): sha256 `token_hash`, 7-day `expires_at`, `accepted_at`/`accepted_method`/`revoked_at`, one open row per user, tenant-isolation RLS plus a SELECT-only policy matching exactly one presented hash through `app.invite_token_hash` (`set_invitation_token_lookup`); no `app.rls_bypass`. Legacy `users.password_hash = 'invite:<token>'` rows on inactive users are backfilled with a fresh window; `password_hash` itself is not modified (a later contract migration clears it). `invite_user` had stored the raw token in `password_hash` and emailed a `/reset-password?invite=1` link that `reset_password` could not redeem, so no invited person could get in.
- `invite_user` creates the invitee with no credential, lowercases the address (the duplicate check was case-sensitive), and HTML-escapes the admin-typed names it puts in the email. New `POST /api/admin/users/{id}/invitation/resend` and `DELETE /api/admin/users/{id}/invitation`; `deactivate_user` revokes an open invitation; `GET /api/admin/users` reports `invitation_status` (`pending`/`expired`/`revoked`) and `invitation_expires_at`. Tightening: `reactivate_user` returns 409 for a person who never accepted, since it previously activated an account with no credential that Google verified-email matching would then sign in.
- New public `POST /api/auth/invite/lookup` (read-only) and `POST /api/auth/invite/accept` (password), rate-limited with reset-password and allowlisted in the route-auth coverage test. Acceptance claims the invitation with a conditional `UPDATE … WHERE accepted_at IS NULL AND revoked_at IS NULL AND expires_at > now()`, so a link works once under concurrent requests.
- `GET /api/auth/{google,microsoft}/login` accept `invite` (checked before redirecting, carried in OAuth state, never combined with signup). A callback carrying an invitation links the identity only when the token is valid and the provider reports the invited address (Google verified email; Microsoft `email` or `preferred_username`), and only if no other user holds that identity (`uq_users_entra_identity` included). All checks run before the claim. Without an invitation the callback path is unchanged and Microsoft still never links by email.
- `AuthRefusal` (an `HTTPException` subclass with a `code`) replaces expected sign-in refusals with the same status and detail; `require_active_tenant` raises it too, so JSON callers are unaffected. The four browser OAuth routes redirect refusals and other 4xx to `/login?error=<allowlisted code>` with `Cache-Control: no-store`; 5xx are re-raised. Only the code is logged.
- `forgot_password` sends nothing (same generic response) for invitees and inactive accounts; `reset_password` refuses an inactive or invite-pending account before any change. A reset never activates anyone.
- Frontend: `AcceptInvitePage` at `/accept-invite` (password or provider, token removed from the address bar), `LoginPage` explains `?error=` codes from `utils/loginErrors.js` without rendering URL text, `/reset-password?invite=1` redirects to the accept page, and the admin Users list shows invitees with Resend/Revoke instead of the active toggle.

## 2026.09.15.04 — Conversation AI preferences, client document access, sample-form provenance

- Store an assistant conversation's tier and public-case-law choice on the row (`conversations.use_premium_llm`, `conversations.include_public`, migration `190_conversation_ai_preferences`, both defaulted to the previous in-memory values). `POST`/`PATCH`/`GET /api/conversations` carry them, `PATCH` accepts either field alone, and a send records the requested pair so the stored value tracks the last turn. `ChatPage` hydrates from the conversation on open, saves each toggle, and seeds a conversation it creates with what is on screen; it had held both in `useState` and never read them back, so any remount reset them (#487).
- Read the tenant `include_public_case_law` setting in the chat path for the first time, through one reader (`app/services/public_case_law_policy.py`). Firm policy narrows a stored preference and never widens it, the stored value stays the user's own choice, and absent or malformed config means allowed. `GET /api/auth/me` returns `public_case_law_allowed`; the chat switch shows off and disabled with the reason instead of a preference retrieval ignores. Tier still fails closed: `reject_demo_premium`, `premium_ai_enabled` and `resolve_llm_route` apply on every turn, and a demo workspace cannot store a premium preference either. Precedence is documented in `docs/assistant-conversation-settings.md`.
- Derive every shared-document count and label from one definition (`app/services/portal_document_access.py`): `firm_shared`, `signing_packet`, `client_upload`. The portal overview now counts exactly what the documents tab lists and breaks the total down (`firm_shared_document_count`, `signing_document_count`, `client_upload_count`), each portal document carries `access_source`, and the client's documents tab groups signing paperwork separately. A per-recipient signing grant cannot be represented by `portal_visible`, which is why the counts could not be reconciled by picking one of them (#489).
- Add `signing_access` to `MatterDocumentResponse`, resolved in `serialize_documents` from the matter's intake packets whose invite is not revoked. The staff documents tab shows "Available to signing client" where it is true instead of "Private"; the share toggle is unchanged. Verified and documented in `docs/client-document-visibility.md`: a signing grant does widen visibility beyond the shared set by design — a grant-only document is listed and served to that recipient — and a document that is neither shared nor granted is still not listed, counted, or downloadable.
- Add `sample_templates.provenance` (migration `191_sample_template_provenance`) and a documented manifest `provenance` block (`source_name`, `source_url`, `edition`, `retrieved_at`, `source_files`); `build_sample_template_library.py` carries it from the import library's `catalog.json`, which the seeder validates and the catalog API returns. Values stay null until the next rebuild against that source, which is not in this repository. The Template Studio library numbers same-titled forms by field count, shows provenance where recorded, and says the source was not recorded where it is not, rather than inventing an edition label on the page where a form is picked for filing (#504). `backend/seed/sample_templates/README.md` documents the schema and the four duplicate titles.

## 2026.09.15.03 — AcroForm parity: required flags, signing classification, bindings

- `build_library_intake_forms.py` passes explicit `fieldFlags` on `checkbox` (empty) and `radio` (`radio`). reportlab defaults them to `required` and `noToggleToOff required radio`, so all 130 checkboxes across the three authored library forms carried `/Ff` bit 2. `fill_pdf_template` treats a required checkbox as unsatisfied unless it is checked, so with `enforce_required=True` — activation preview, matter-bound generation and the document workspace — none of the three could be generated, and `existing_case_yes`/`_no`/`_unsure` made the requirement unsatisfiable. The seed PDFs are rebuilt with zero required fields and `build_form()` now fails the build if any field comes back required.
- A source-required checkbox can be made optional in review; a source-required text field still cannot. Both `PATCH /templates/{id}` review reconciliation and the renderer's `required_variables` map apply the rule, since the renderer re-read `required` from the live PDF and so ignored the stored schema. `discover_pdf_fields` emits `source_required` alongside `required`, carried through review as an authoritative key, and `PrepareFormWorkspace` disables the Required control with an explanation where the requirement genuinely cannot be lifted instead of offering one that silently reverted.
- `esign/plan.py` replaces the `_ACROFORM_INITIALS` substring match with `_is_initials_field()`, which tests a field's name and label against separate patterns and vetoes both on name-part wording. A text field named `client_name` labelled "Last Name, First Name, Middle Initial" was promoted to `kind="initials"` and planned as a signing widget over the client's own name box.
- `render_pdf_page_preview()` calls `pdfium` `init_forms()` before rendering. pdfium draws widget appearances only through a form-fill environment, so every AcroForm field was omitted from the server-rendered page despite carrying `/AP`; the shipped questionnaire went from 73,982 to 638,354 non-background pixels.
- `template_bindings` adds ten `client.*` paths for columns the contact record already stores: `date_of_birth`, `secondary_phone`, `preferred_contact_method`, `preferred_contact_window`, `preferred_language`, `referral_source`, and the four `emergency_contact` JSON keys. `_collect_smart_fill_candidates` resolves them and the probe contact in `_smart_fill_alias_vocabulary` carries the attributes so the approval check stays in sync. `_address_value` is renamed `_mapping_value`.
- `build_library_intake_forms.py` gains `RADIO(...)` and `Choice(...)`; converting the shipped forms' yes/no checkbox groups is deferred because it renames fields.
- `app/services/__init__.py` and `app/services/esign/__init__.py` resolve their re-exports through PEP 562 `__getattr__`. Importing `app.services.pdf_templates` — pypdf plus a stdlib-only placement module — pulled in the embeddings stack, SQLAlchemy, pgvector, Stripe, Redis, tiktoken and the JWT middleware, so any build or analysis script needed a full runtime to read a form field.
- `build_wcbls_client_forms.py` calls the engine's own initials classifier instead of restating its patterns, and the tenant form's name field is back to the firm's "Last Name, First Name, Middle Initial".

## 2026.09.15.02 — Existing engagements, open dates, and bulk matter creation

- Add `matters.opened_on` (migration `189_matter_engagement`, on top of main's `188_completion_escalation`), backfilled from `created_at` and defaulting to `CURRENT_DATE`; `POST /api/matters` and `PATCH /api/matters/{id}` accept it (past or today only), lists, detail, the client-matters payload and the Workspace MCP matter snapshot read it, and default retention runs seven years from it. Workflow synthesis keeps anchoring observations on the row's `created_at`, since seeded and rehearsal matters backdate that column without an open date.
- Add an engagement record on the matter (`engagement_status` in `signed_on_file`, `signed_no_copy`, `no_agreement`, `pending_copy`, plus signing date, document, note, and who recorded it). `POST /api/matters/{id}/engagement` files a signed fee agreement (uploaded PDF or existing matter document, stored as a `contract` filed copy) or records a signed agreement with no copy or no agreement with its reason, moves an unset or intake/transfer stage to Active, writes a system event, and creates no portal invitation, signature request or message. A record without a copy can be upgraded by adding one; contradicting a filed agreement needs `replace`. A verified intake receipt of the fee agreement stamps the same record. `GET /api/matters` filters by `engagement_status`.
- New Matter: open date input, an Engagement choice (new, or already engaged with the three answers above), and a retry that never opens a second matter when recording fails. Paperwork card: "Already engaged — record it", the recorded engagement with its signed copy, "Add signed copy", and a fee-agreement hint in the drawer. Portfolio: open date from `opened_on`, engagement badge and filter. Matter details: Opened field and editor.
- Folder importer mappings accept `opened_on`, `agreement`, `agreement_signed_on` and `agreement_note`; a signed date without the file records `pending_copy`.
- Bulk creation from CSV: `GET /api/matter-imports/csv/template`, `POST /api/matter-imports/csv` (preview: 1 MiB, 500 rows, per-row client/attorney resolution and problems, staged as an `ExternalImportRun` with provider `matter_csv_v1`), `POST /api/matter-imports/csv/{id}/confirm` (one transaction: contacts reused by contact ID, client number or email and otherwise created, matters with numbers, assignments, events, engagement and provenance links; rows with problems or a stale preview are refused; a repeat returns the stored result). No automations, cloud folders or client messages. New Matter gains the "Bulk create from CSV" mode with template download, row review and results.
- Move CSV decoding and formula-lead hardening to `app/services/csv_upload.py`; the client CSV routes use it.

## 2026.09.15.01 — Matter action labels, task due times on provider calendars, and signing evidence

- Rename the My Matters assignment control to "Work on this" / "Working on this", expose it as an `aria-pressed` toggle, and move it off the green lifecycle palette onto the accent palette with a person icon. "Active" had named the status filter tab, the `StatusBadge`, and this per-assignment `is_active_working` flag on one screen, and the flag's label read as a state change it never made. The board card and the list row now render one shared `WorkingToggle`.
- Replace the decorative `View →` span on a My Matters row with a real link to the matter that reveals on focus as well as hover and names the matter it opens. It had no `href`, no handler and no role, and the row had no handler either.
- Thread a task's `due_time` through `push_task_to_calendars` and build a timezone-aware timed event in both providers instead of forcing `isAllDay` (Microsoft) and a date-only `start` (Google). The timezone is the matter's intake timezone, which is already what follow-up due times are written in; a task with no matter falls back to UTC. A task with no due time stays all-day.
- Surface the `clarity_task_id` marker on both calendar read paths so `mergeCalendarEvents` collapses a synced copy into the LawHand task instead of rendering both, and link the task event to `/tasks/{id}` rather than the task list. A Graph rejection of the extended-property `$expand` now falls back to a plain read rather than failing the whole calendar view.
- Count consecutive filing failures on a signature request and, after 12 attempts spanning at least an hour, raise one assigned matter follow-up task to the request creator. Retrying continues; a later success clears the run and closes the task. Adds `completion_failure_count`, `completion_first_failed_at` and `completion_escalated_at` (migration `188_completion_escalation`). The retry contract is documented in `docs/operations/signing-filing-retry-contract.md`.
- Resolve the signer address on the signing paths through the shared trusted-proxy resolver (`app/utils/client_address.py`, `TRUSTED_PROXY_HOPS`) instead of `request.client.host`, which behind nginx is our own container network. Evidence never falls back to the peer: an unresolved address renders as "not attributable" with a footnote and records `ip_source` on the audit. Rate limiting keeps its peer fallback. Certificates already issued are not rewritten.
- Classify Twilio rejections by whose fault they are on the platform shared SMS sender: a 4xx from Twilio is a caller error carrying Twilio's own message and error code, a timeout is 504, and only Twilio being unavailable stays 502. Every non-2xx had become a 502, which an edge proxy replaces with its own error page, so the message never reached the operator. Account and Messaging Service SID shapes are validated at save and on resolve, naming a Verify (`VA…`) SID pasted for a Messaging Service (`MG…`) SID; error `detail` is now structured.
...
- Migration 186 adds fee/schedule tables with forced RLS, invoice generation keys/details and isolated platform subscription records.
