...
def test_release_catalog_returns_latest_release_and_history():
    ...
    assert latest["title"] == "Guides with screenshots and step-by-step help"
    assert latest["highlights"][0]["title"] == "Screenshots with numbered steps"
    assert latest["highlights"][1]["title"] == "Guide opens the right section"
    assert latest["highlights"][2]["title"] == "Help inside panels"
