...
LATEST_RELEASE_ID = "2026.09.22.09"
...
def test_release_catalog_returns_latest_release_and_history():
    ...
    assert latest["title"] == "Clearer recovery for matter storage"
    assert latest["highlights"][0]["title"] == "Find a missing matter folder"
    assert latest["highlights"][1]["title"] == "Set up a folder, then retry"
    assert latest["highlights"][2]["title"] == "Separate provider status from folder access"
    history_ids = [release["id"] for release in catalog["release_notes"]]
    assert history_ids[:4] == [LATEST_RELEASE_ID, "2026.09.22.08", "2026.09.22.07", "2026.09.22.06"]
