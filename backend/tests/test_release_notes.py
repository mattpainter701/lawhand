...
LATEST_RELEASE_ID = "2026.09.25.01"
...
def test_release_catalog_returns_latest_release_and_history():
    ...
    assert latest["title"] == "Edit matter documents in Word or Google Docs"
    assert latest["highlights"][0]["title"] == "Open in Word or Google Docs"
    assert latest["highlights"][1]["title"] == "Bring back changes"
    assert latest["highlights"][2]["title"] == "Nothing is silently overwritten"
    history_ids = [release["id"] for release in catalog["release_notes"]]
    assert history_ids[:4] == [LATEST_RELEASE_ID, "2026.09.24.03", "2026.09.24.02", "2026.09.24.01"]
