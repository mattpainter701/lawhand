...
LATEST_RELEASE_ID = "2026.09.24.03"
...
def test_release_catalog_returns_latest_release_and_history():
    ...
    assert latest["title"] == "Fill forms on the document itself"
    assert latest["highlights"][0]["title"] == "The document comes first"
    assert latest["highlights"][1]["title"] == "Review on the page"
    assert latest["highlights"][2]["title"] == "Guided answers on any screen"
    assert latest["highlights"][3]["title"] == "Switch between Document, Questions and Preview"
    history_ids = [release["id"] for release in catalog["release_notes"]]
    assert history_ids[:4] == [LATEST_RELEASE_ID, "2026.09.24.02", "2026.09.24.01", "2026.09.23.02"]
