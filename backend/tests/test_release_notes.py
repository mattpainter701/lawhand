...
LATEST_RELEASE_ID = "2026.09.22.07"
...
def test_release_catalog_returns_latest_release_and_history():
    ...
    assert latest["highlights"][0]["title"] == "See your answers without another click"
    history_ids = [release["id"] for release in catalog["release_notes"]]
    assert history_ids[:2] == [LATEST_RELEASE_ID, "2026.09.22.06"]
