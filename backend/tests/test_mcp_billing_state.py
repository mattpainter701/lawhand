...
def test_masking_is_for_api_responses_not_logs():
    """`_mask` keeps a correlatable prefix and suffix for an authenticated
    caller. It is deliberately absent from every log statement in this module:
    logs are shipped and retained, and a truncated Stripe id still carries most
    of its distinguishing characters. Log lines name the tenant id or the
    Stripe event id instead.
    """
...
def test_no_stripe_identifier_reaches_a_log_statement():
    import ast
    import inspect

    source = inspect.getsource(billing)
    tree = ast.parse(source)
    sensitive = {"customer_id", "subscription_id", "deleted_subscription_id",
                 "current_subscription_id", "stripe_payment_intent_id"}
    ...
    for node in ast.walk(tree):
        ...
        for arg in ast.walk(node):
            if isinstance(arg, ast.Name) and arg.id in sensitive:
                offenders.append((func.attr, arg.id))
