...
<div class="wrapper">
  {content}
  <div class="footer">
    LawHand &bull; This is an automated notification &bull;
    {timestamp}
  </div>
</div>
</body>
...
def _build_renewal_alert_html(alerts: List[dict]) -> str:
    ...
    content = f"""
    <div class="header">
      <h1>LawHand &mdash; Renewal Alerts</h1>
...
def _build_agent_digest_html(agent_name: str, digest_markdown: str) -> str:
    ...
    content = f"""
    <div class="header">
      <h1>LawHand &mdash; {agent_name}</h1>
...
def _build_oc_status_html(
    ...
    content = f"""
    <div class="header">
      <h1>LawHand &mdash; Weekly Portfolio Status</h1>
...
class EmailService:
    ...
    async def send_renewal_alert(
        ...
        subject = f"LawHand — {len(alerts)} Contract Renewal Alert(s)"
    ...
    async def send_agent_digest(
        ...
        """
        subject = f"LawHand — {agent_name} Weekly Digest"
    ...
    async def send_oc_status(
        ...
        subject = f"LawHand — Weekly Portfolio Status: {tenant_name}"
...
async def send_portal_invite(
    ...
    html_body = _BASE_HTML.format(content=content, timestamp=now_str)
    text_body = (
        f"You've been invited to the LawHand mediation portal for "
        f"'{case_name}'.\n\nAccess it here: {invite_url}\n"
    )
    return await email_service.send_email(
        to=[to_email],
        subject=f"LawHand — Mediation Portal Invitation: {case_name}",
        html_body=html_body,
        text_body=text_body,
    )
