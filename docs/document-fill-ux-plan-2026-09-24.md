...
1. ~~Phase 2 next?~~ Yes. Built; in review.
...
## 10. Option A: build plan

Open a matter's Word document in the firm's Word or Google Docs, and bring the
...
- **Matter documents** (`backend/app/models/matter_document.py`) already store
  ...
  - `storage_provider` and `storage_backend` (onedrive / sharepoint /
    google_drive / local);
  - `provider_object_id` and `provider_drive_id`;
  - `provider_etag` and `provider_version_id`;
  - `document_sha256` and `storage_state` (verified / conflict / …);
  - `document_status` (draft / in_review / approved / filed / superseded).
- **Template saves don't record everything.** `document_templates.py`
  `_storage_document_fields` does not persist the eTag, version or checksum
  that the store returns, and leaves `storage_state` unset.
- **Reusable parts:**
  - `MatterFileStore.read_matter_file_bytes`;
  - `CloudArtifactMaterializer.read_current_cloud_bytes` (reads current
    bytes by durable id);
  - `inspect_cloud_docx_snapshot` (rejects unsafe DOCX and returns
    sha/size/text);
  - `get_matter_file_open_url` (fresh provider link);
  - `append_document_integrity_event`.
- **Not reusable as-is:** the AI-draft sync (`tasks.py` sync-cloud,
  `materialize()`) is tied to artifacts and tasks, and copies the bytes to a
  new cloud object. For matter documents that would detach the file the
  person has open in Word.
- **D08 is live.** After any edit in Word or Drive, download, e-sign, the MCP
  read and fact extraction fail with 409 and mark the document `conflict`,
  because the size or hash no longer match. There is no endpoint that
  reconciles a plain matter document.
- **D23:** the Documents tab has no SharePoint label and falls back to "Local".

### Design

- **Adopt in place.** Bringing back changes updates the same MatterDocument:
  sha, size, eTag, version, modified time, and `storage_state=verified`. The
  cloud file the person has open stays the one LawHand points to. Each
  adoption writes an integrity event (`external_cloud_edit_adopted`) with the
  previous and new sha, plus a matter timeline event. Those events are the
  version history.
- **Approved, filed and signing-bound documents are not overwritten.**
  - If the cloud copy of an `approved` or `filed` document changed, the result
    is "changed in Word after approval". The document stays in `conflict`
    until someone saves the edit as a new document (it supersedes the old one)
    or restores the cloud file.
  - An adopted edit on a draft or in-review document keeps its status; review
    state resets by clearing the recorded approval.
- **Checkout marker.** "Open in Word/Docs" records who opened it and when.
  The row shows "Being edited in Word by Ada since 2:14 pm". Returning to the
  tab, or choosing **Bring back changes**, reconciles and clears the marker.
  The marker is informational: the firm's suite handles its own co-authoring,
  so LawHand does not block it.
- **Open links.**
  - Word for the web: the item's `webUrl`, which opens it for editing.
  - Word desktop: `ms-word:ofe|u|<url>`.
  - Google Docs: the file's `webViewLink`; Docs edits a DOCX in Office mode
    and keeps it DOCX.
  - All links come from a fresh provider read, never the stored display URL.
- **Fallback: Upload revised version.** Accepts a DOCX and runs the same
  safety inspection. It writes the bytes to the same cloud item (or local
  path), then adopts them exactly as above. It covers staff without their own
  connection, and desktop edits made offline.
- **The first slice leaves the rest alone.** No change subscriptions (polling
  on focus is enough), no Word add-in, and no changes to the AI-draft path.
  D34 (checking the cloud eTag before a LawHand text save) and D09 follow in
  the next slice.
