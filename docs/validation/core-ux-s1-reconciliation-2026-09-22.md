...
| G-16 | Local save shown as success when provider sync fails | **Reproducible** | `CalendarPage.jsx:751` still sets `Event created.` unconditionally; no `sync_status`/`sync_error` read | Keep in scope (S1.09); not fixed here |
...
All other audit rows are **unaffected** by this range: the relevant source paths did not change between `c4d80b46` and `ead233f0`, so the audit snapshot still describes them.
