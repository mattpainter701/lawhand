...
| `This template's signing fields would not bind ... give a signer role to ...` on publish or activate | A signature, initials, or signing-date field carries no signer role | Open the template, select each named field, and set its signer role. The role is what ties the field to a signer when the document is sent. |
