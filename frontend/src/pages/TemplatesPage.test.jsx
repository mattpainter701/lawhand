...
vi.mock('../api', () => ({
  ...
  getTemplateCards: vi.fn().mockResolvedValue({ cards: [], operators: [] }),
...
describe('document template workflow', () => {
  ...
  it('loads a global sample into ordinary review and creates only an explicit firm draft', async () => {
    ...
    try {
      ...
      expect(screen.getByText('Shared source selected')).toBeVisible()
      expect(screen.getByText('source.pdf')).toBeVisible()
      expect(screen.getByRole('button', { name: 'Choose a different source document' })).toBeVisible()
      expect(screen.queryByText(/Current source:/)).not.toBeInTheDocument()
      expect(screen.getByText('Save draft')).toBeVisible()
      expect(screen.getByText('Review fields in editor')).toBeVisible()
  ...
  it('keeps existing placeholders intact when selected words occur inside an automation key', async () => {
    analyzeTemplateUpload.mockResolvedValue({ title: 'Application', format: 'docx', body: 'Applicant: {{client_name}}; role: client.', extracted_text: 'Applicant: Ada; role: client.', suggested_variable_schema: { source: 'docx_source', fields: [{ name: 'client_name', label: 'Client name', source_text: 'Ada' }] }, warnings: [] })
    const user = userEvent.setup()
    render(<TemplatesPage />)
    await user.click(await screen.findByRole('button', { name: 'Upload Sample' }))
    fireEvent.change(screen.getByLabelText('Sample document'), { target: { files: [new File(['word'], 'application.docx')] } })
    await screen.findByDisplayValue('Application')
    await user.click(screen.getByRole('button', { name: 'Fields', exact: true }))
    const source = screen.getByLabelText('Select source text')
    const start = source.firstChild.textContent.lastIndexOf('client')
    const range = document.createRange()
    range.setStart(source.firstChild, start)
    range.setEnd(source.firstChild, start + 6)
    window.getSelection().removeAllRanges()
    window.getSelection().addRange(range)
    fireEvent.mouseUp(source)
    await user.click(screen.getByRole('button', { name: 'Make selection a field' }))
  ...
  it('converts a Word template to a review-bound PDF for signature', async () => {
    getTemplates.mockResolvedValueOnce({ items: [{
      id: 'signature-docx', title: 'Signature Packet', body: 'Dear {{client_name}}', category: 'engagement_letter',
      format: 'docx', source_filename: 'signature.docx', source_sha256: 'abc', is_active: true,
      variable_schema: { fields: [{ name: 'client_name', label: 'Client name', source_text: 'Ada Lovelace' }] },
    }] })
    const pdfBlob = new Blob(['%PDF-1.7'], { type: 'application/pdf' })
    renderTemplateFile.mockResolvedValue({
      blob: pdfBlob,
      filename: 'Signature_Packet.pdf',
      contentType: 'application/pdf',
      previewId: 'docx-pdf-preview',
      previewPurpose: 'generation',
    })
    URL.createObjectURL = vi.fn().mockReturnValue('blob:docx-pdf-preview')
    URL.revokeObjectURL = vi.fn()
    const user = userEvent.setup()
    render(<TemplatesPage />)
