...
const MARKETING_SHELLS = Object.freeze({
  ...
  '/requirements': {
    ...
    sections: [
      ...
      { heading: 'Microsoft 365', body: 'Tenant-wide consent is granted by an Entra Global Administrator and covers directory read, Outlook mail read and send, OneDrive and SharePoint file read/write, and calendar read/write. Firms can connect per user instead, which drops the directory scope, makes user matching manual, and asks only to read files. Microsoft Teams scopes (read teams and channels, post channel messages) are added only on explicit opt-in.' },
