...
export default function TemplateTestSummary({ template, error, rendering = false, outputReady, missing = [], diagnostic = false, onFixFields }) {
  ...
  const rows = [
    ...
    { label: 'Field names', status: invalid.length ? 'Needs fixing' : 'Passed', detail: invalid.length ? `Give these fields unique, valid names: ${invalid.map(field => field.label || field.name || 'Unnamed field').join(', ')}.` : `${fields.length} included field${fields.length === 1 ? '' : 's'} with valid, unique names. Confirm their replacement locations in Workspace.` },
    ...(missing.length ? [{ label: 'Test values', status: 'Needs fixing', detail: `Enter values for: ${missing.map(fieldLabel).join(', ')}.` }] : []),
    { label: 'Generate output', status: rendering ? 'Running' : failed ? 'Needs fixing' : passed ? diagnostic ? 'Preview ready' : 'Passed' : 'Not tested', detail: error || (failed ? 'The latest test failed. Run it again to see the current error.' : passed ? diagnostic ? 'Draft output generated. Run the representative test to record publication evidence.' : `Output generated for version ${template.current_version_no || 'the current draft'}.` : 'Run a test with sample values. An earlier version’s result does not test this draft.') },
