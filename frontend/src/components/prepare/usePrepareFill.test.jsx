...
describe('usePrepareFill automatic fill and preview', () => {
  ...
  it('recovers expired save evidence with an explicit preview retry, preserving answers', async () => {
    const { result } = renderFill()
    await pause(0)
    act(() => result.current.setVariable('client_name', 'Reviewed client'))
    await pause(700)
    renderTemplate.mockRejectedValueOnce({ response: { status: 409, data: { detail: 'The PDF preview expired. Preview the exact current values again before save.' } } })
    await act(async () => result.current.handleSave())
    expect(result.current.previewId).toBe('')
    expect(result.current.previewError).toContain('preview expired')
    expect(result.current.filePreview).not.toBeNull()
    expect(result.current.variables.client_name).toBe('Reviewed client')
    await pause(1400)
    expect(renderTemplateFile).toHaveBeenCalledTimes(1)
    expect(renderTemplate).toHaveBeenCalledTimes(1)
    renderTemplateFile.mockResolvedValueOnce(previewResult('renewed-preview'))
    await act(async () => result.current.handleRender())
    expect(result.current.previewError).toBe('')
    expect(result.current.previewId).toBe('renewed-preview')
    expect(renderTemplate).toHaveBeenCalledTimes(1)
    await act(async () => result.current.handleSave())
    expect(renderTemplate).toHaveBeenLastCalledWith('template-1', expect.objectContaining({ preview_id: 'renewed-preview', variables: { client_name: 'Reviewed client' } }))
    expect(result.current.saved).toBe(true)
  })

  it('keeps current preview evidence for a storage failure so only Save needs retrying', async () => {
    const { result } = renderFill()
    await pause(0)
    act(() => result.current.setVariable('client_name', 'Reviewed client'))
    await pause(700)
    renderTemplate.mockRejectedValueOnce({ response: { status: 503, data: { detail: 'Storage is unavailable.' } } })
    await act(async () => result.current.handleSave())
