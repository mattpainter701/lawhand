...
export default function InvoicePaymentPlan({ invoice, onSaved }) {
  ...
  return <section className="my-4 rounded-xl border border-brand-line p-4" aria-label="Payment plan">
    ...
    {rows.map((row, index) => <div key={index} className="mt-2 flex flex-wrap gap-3">
      <label className="text-sm">Installment date {index + 1}<input className="min-h-11 rounded-xl border border-brand-line bg-brand-surface px-3 text-sm text-brand-ink focus:outline-none focus:ring-2 focus:ring-brand-accent block" type="date" min={invoice.issue_date} disabled={!editable || busy} value={row.due_date} onChange={e => setRows(all => all.map((r, i) => i === index ? { ...r, due_date: e.target.value } : r))} /></label>
      <label className="text-sm">Installment amount {index + 1}<input className="min-h-11 rounded-xl border border-brand-line bg-brand-surface px-3 text-sm text-brand-ink focus:outline-none focus:ring-2 focus:ring-brand-accent block" type="number" min="0.01" step="0.01" disabled={!editable || busy} value={row.amount} onChange={e => setRows(all => all.map((r, i) => i === index ? { ...r, amount: e.target.value } : r))} /></label>
