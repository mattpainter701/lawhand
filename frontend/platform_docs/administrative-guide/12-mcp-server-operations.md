...
- **Reads:** `search_clients`, `get_client`, `search_intakes`, `get_intake`, `search_matters`, `find_matter`, `get_matter_context`, `search_tasks`, `search_firm_memory`, `get_task`, `list_matter_tasks`, `list_matter_recipients`, `list_matter_documents`, `get_matter_document_text`, `list_document_templates`, `get_document_template_text`, and `get_workflow_run` (a workflow's checkpoints, review tasks, and outcomes, without work-product bodies).
- **Proposals:** `propose_task`, `propose_task_update` (a status, assignee, priority, due-date, or note change to an existing task), `propose_client_email`, `propose_client_sms` (only to verified, consented matter parties; the client must also send a stable idempotency key), and `propose_matter_document`; `propose_document_from_template` fills an active Word or Markdown firm template into the same review.
- **Workflows:** `propose_workflow_run` starts a bounded matter workflow built from the same read and propose tools, pausing for missing input and human review, and `resume_workflow_run` continues a paused run with only the inputs it asked for. Approval of every step stays in LawHand.
